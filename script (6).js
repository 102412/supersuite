...
const LiveSync = {
  // ── Channels ─────────────────────────────────────────────────────
  _bc: null,       // BroadcastChannel  — same browser, instant
  _ws: null,       // WebSocket         — cross-device, cross-machine
  _wsReady: false, // true once WS open and handshake complete

  // ── State ─────────────────────────────────────────────────────────
  sessionCode: null,
  isOwner: false,
  _syncInProgress: false,  // guard: prevents echo-back when applying remote state
  _lastAppliedTs: 0,       // timestamp of last state we applied from any peer
  _pendingPushTs: 0,       // timestamp of our last outgoing push
  _syncTimeoutId: null,    // timer for stuck-syncing recovery
  _wsReconnectDelay: 1000, // exponential back-off start (ms)
  _wsReconnectTimer: null,
  _wsHeartbeatTimer: null,

  // ── WebSocket relay config ─────────────────────────────────────────
  // PieSocket public demo endpoint — works in any browser, no account needed.
  // For production, replace WS_URL with your own relay (Ably, Supabase, custom).
  WS_URL(code) {
    // wss://s16132.blr1.piesocket.com/v3/{channel}?api_key=demo&notify_self=0
    // Channel = "ss_" + code so Supersuite sessions don't collide with other apps
    return `wss://s16132.blr1.piesocket.com/v3/ss_${code}?api_key=demo&notify_self=0`;
  },

  // ─────────────────────────────────────────────────────────────────
  init(code, isOwner) {
    this.destroy(); // close any existing connections cleanly

    this.sessionCode = code;
    this.isOwner = isOwner;
    this._lastAppliedTs = 0;
    this._pendingPushTs = 0;
    this._syncInProgress = false;
    this._wsReconnectDelay = 1000;

    // Layer 1: BroadcastChannel (same browser / same machine — instant)
    try {
      this._bc = new BroadcastChannel('supersuite_collab_' + code);
      this._bc.onmessage = (ev) => this._onMessage(ev.data, 'bc');
      console.log('[LiveSync] BroadcastChannel open for code:', code);
    } catch(e) {
      console.warn('[LiveSync] BroadcastChannel not available:', e.message);
    }

    // Layer 2: WebSocket (cross-device / cross-machine — true real-time)
    this._connectWS(code);

    this._updateUI(true);
    console.log('[LiveSync] Session started:', code, isOwner ? '(owner)' : '(guest)');
  },
  ...
  _connectWS(code) {
    ...
    let ws;
    try {
      ws = new WebSocket(this.WS_URL(code));
    } catch(e) {
      console.warn('[LiveSync] WebSocket connect failed:', e.message);
      this._scheduleWSReconnect(code);
      return;
    }
    ...
    ws.onopen = () => {
      ...
      this._wsReady = true;
      this._wsReconnectDelay = 1000; // reset backoff
      this._flashSyncIndicator('synced');

      // Heartbeat: send a ping every 25s to keep the connection alive
      // PieSocket (and most relays) close idle connections after 30s
      ...
      // If we're a joiner, request full state from owner immediately
      if (!this.isOwner) {
        setTimeout(() => {
          this._broadcast({ type: 'REQUEST_SYNC', code });
        }, 200);
      }
    };
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        this._onMessage(data, 'ws');
      } catch(e) {
        // Not JSON (e.g. server control message) — ignore
      }
    };
    ...
    ws.onclose = (ev) => {
      console.log('[LiveSync] WebSocket closed, code:', ev.code);
      this._wsReady = false;
      clearInterval(this._wsHeartbeatTimer);
      // Auto-reconnect unless we deliberately destroyed
      if (this.sessionCode === code) {
        this._scheduleWSReconnect(code);
      }
    };
  ...
  _scheduleWSReconnect(code) {
    const delay = Math.min(this._wsReconnectDelay, 30000);
    this._wsReconnectDelay = Math.min(delay * 2, 30000);
    console.log(`[LiveSync] Reconnecting in ${delay}ms…`);
    this._wsReconnectTimer = setTimeout(() => {
      if (this.sessionCode === code) this._connectWS(code);
    }, delay);
  },
  ...
  // ─────────────────────────────────────────────────────────────────
  /** Broadcast a message on ALL available channels */
  _broadcast(payload) {
    const json = JSON.stringify(payload);
    if (this._bc) {
      try { this._bc.postMessage(payload); } catch(e) {}
    }
    if (this._ws && this._wsReady && this._ws.readyState === WebSocket.OPEN) {
      try { this._ws.send(json); } catch(e) {}
    }
  },

  // ─────────────────────────────────────────────────────────────────
  /** Serialize and broadcast full state to all peers */
  push() {
    if (!this._bc && (!this._ws || !this._wsReady)) return; // no channels open
    if (this._syncInProgress) return; // don't echo received state back

    const ts = Date.now();
    this._pendingPushTs = ts;
    ...
    try {
      ...
      console.error('[LiveSync] State serialization failed:', e);
      return;
    }
    ...
    this._broadcast({
      type: 'STATE_UPDATE',
      ts,
      sender: this.isOwner ? 'owner' : 'guest',
      state: stateSnapshot,
    });

    // Show syncing, then self-resolve after 800ms
    // (sender never receives its own message back on WS via notify_self=0
    //  and BroadcastChannel doesn't echo to sender either)
  ...
  _onMessage(data, source) {
    ...
    // Pong / server control messages
    if (data.type === 'PING' || data.type === 'PONG') return;

    // A joining peer (or reconnecting peer) is requesting full state
    if (data.type === 'REQUEST_SYNC') {
      console.log('[LiveSync] Peer requested sync via', source, '— pushing state');
      setTimeout(() => this.push(), 80);
      return;
    }

    if (data.type !== 'STATE_UPDATE') return;

    // Ignore stale messages (allow 100ms clock skew tolerance)
    if (data.ts <= this._lastAppliedTs - 100) {
      console.log('[LiveSync] Dropping stale update ts=' + data.ts);
      return;
    }

    // Don't apply our own echo (can happen if notify_self is on)
    if (data.ts === this._pendingPushTs) return;
    ...
    try {
      ...
      // Full authoritative replace — incoming state is the truth
      if (Array.isArray(s.blocks)) {
        State.blocks = JSON.parse(JSON.stringify(s.blocks));
      }
      if (s.globalStyles && typeof s.globalStyles === 'object') {
        State.globalStyles = JSON.parse(JSON.stringify(s.globalStyles));
      }
      if (s.currentTemplate) State.currentTemplate = s.currentTemplate;
      if (typeof s.customCSS === 'string') State.customCSS = s.customCSS;
      if (s.blockIdCounter > State.blockIdCounter) State.blockIdCounter = s.blockIdCounter;

      refreshPreview();
      updateLayers();
      this._flashSyncIndicator('synced');
      console.log('[LiveSync] Applied state from peer via', source, 'ts=' + data.ts);
    } catch(e) {
      console.error('[LiveSync] Failed to apply remote state:', e);
  ...
  _flashSyncIndicator(state) {
    ...
    if (state === 'syncing') {
      ...
      status.textContent = 'Sync error — retrying';
  ...
  _updateUI(active) {
    ...
    if (info && active) {
      info.textContent = this.isOwner
        ? 'Owner — Code: ' + this.sessionCode
        : 'Collaborator — Code: ' + this.sessionCode;
    }
  },

  /** Call after any local state mutation to broadcast the change */
  notifyChange() {
    if (this._syncInProgress) return; // never echo received state back
    const hasBC = !!this._bc;
    const hasWS = this._ws && this._wsReady && this._ws.readyState === WebSocket.OPEN;
    if (!hasBC && !hasWS) return;
    // Debounce rapid edits (e.g. typing) into a single push
    clearTimeout(this._pushTimer);
    this._pushTimer = setTimeout(() => this.push(), 150);
  },
};
...
function joinCollabSession() {
  ...
  setTimeout(() => {
    if (LiveSync.channel) {
      try { LiveSync.channel.postMessage({ type: 'REQUEST_SYNC', code }); } catch(e) {}
    }
  }, 300);
}
...
function buildPreviewHTML(forExport = false) {
  ...
  const blocksHTML = State.blocks.map(block => {
    ...
    // FIX2: Apply glass tint/blur/opacity per block if configured
    if (block.data.glassTint && block.data.glassTint !== 'none') {
      ...
      const glassCSS = 'box-shadow:inset 0 0 0 2000px ' + block.data.glassTint + ';'
        ...
        + (opac < 1 ? 'opacity:' + opac + ';' : '');
      html = html.replace(/(<(?:section|div|nav|header|footer)[^>]*class="ss-block )/, '$1').replace(/class="ss-block ([^"]*)"/, 'class="ss-block $1" data-glass="1" style="' + glassCSS + '"');
    }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Preview-mode hero normalization: iframe viewport ≠ real browser viewport */
    ...
    body.ss-preview-mode .ss-hero[style*="min-height: 100vh"] { min-height: min(100vh, 600px) !important; }
    body.ss-preview-mode .ss-hero[style*="min-height:85vh"],
    body.ss-preview-mode .ss-hero[style*="min-height: 85vh"] { min-height: min(85vh, 520px) !important; }
    body.ss-preview-mode .ss-hero-orb { display: none !important; }
  </style>
</head>
...
function refreshPreview() {
  ...
  // Stable floor before srcdoc swap — prevents vh hero computing against 0px viewport
  ...
  // Notify collab peers of state change (debounced inside notifyChange)
  ...
  frame.onload = () => {
    try {
      ...
      frame.style.height = Math.max(Math.min(contentH, 8000), 400) + 'px';
    } catch(e) {}
  };
}
...
function renderGlassTab(data) {
  ...
  return `
    ...
    <div class="glass-range-group">
      ...
      <input type="range" min="0" max="48" value="${blur}" oninput="_editingData.glassBlur=parseInt(this.value);document.getElementById('glass-blur-val').textContent=this.value+'px';applyGlassLive()"/>
    </div>
    <div class="glass-range-group">
      <label>Block Opacity <span id="glass-opacity-val">${Math.round(opacity*100)}%</span></label>
      <input type="range" min="20" max="100" value="${Math.round(opacity*100)}" oninput="_editingData.glassOpacity=this.value/100;document.getElementById('glass-opacity-val').textContent=this.value+'%';applyGlassLive()"/>
    </div>
  `;
}
  ...
  return `
    ...
    <div id="hero-content-panel" class="bsm-panel active">
      <div class="bsm-field"><label>Headline</label>
        <textarea rows="2" oninput="_editingData.heading=this.value;heroLiveUpdate()">${data.heading}</textarea>
      </div>
      <div class="bsm-field"><label>Subheading</label>
        <textarea rows="2" oninput="_editingData.subheading=this.value;heroLiveUpdate()">${data.subheading}</textarea>
      </div>
      <div class="bsm-field"><label>Badge Text</label>
        <input type="text" value="${data.badgeText}" oninput="_editingData.badgeText=this.value;heroLiveUpdate()"/>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div class="bsm-field"><label>Show Badge</label>
          <select onchange="_editingData.showBadge=this.value==='true';heroLiveUpdate()">
        ...
        <div class="bsm-field"><label>Alignment</label>
          <select onchange="_editingData.alignment=this.value;heroLiveUpdate()">
      ...
      <div class="bsm-field"><label>Min Height</label>
        <select onchange="_editingData.minHeight=this.value;heroLiveUpdate()">
    ...
    <div id="hero-design-panel" class="bsm-panel">
      <div class="bsm-field"><label>Background Type</label>
        <select onchange="_editingData.bgType=this.value;heroLiveUpdate()">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div class="bsm-color-row"><label>BG Color 1</label>
          <input type="color" value="${data.bgColor}" oninput="_editingData.bgColor=this.value;heroLiveUpdate()"/>
        </div>
        <div class="bsm-color-row"><label>BG Color 2</label>
          <input type="color" value="${data.bgColor2}" oninput="_editingData.bgColor2=this.value;heroLiveUpdate()"/>
        </div>
      </div>
      <div class="bsm-field"><label>Background Image URL</label>
        <input type="url" value="${data.bgImage||''}" placeholder="https://…"
          oninput="_editingData.bgImage=this.value;heroLiveUpdate()"/>
      </div>
      <div class="bsm-color-row"><label>Text Color</label>
        <input type="color" value="${data.textColor}" oninput="_editingData.textColor=this.value;heroLiveUpdate()"/>
      </div>
    </div>
    ...
    <div id="hero-btns-panel" class="bsm-panel">
      ...
      <div class="bsm-field"><label>Text</label><input type="text" value="${data.btnText}" oninput="_editingData.btnText=this.value;heroLiveUpdate()"/></div>
      <div class="bsm-field"><label>Link</label><input type="url" value="${data.btnLink}" oninput="_editingData.btnLink=this.value"/></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div class="bsm-color-row"><label>Btn Color</label><input type="color" value="${data.btnColor}" oninput="_editingData.btnColor=this.value;heroLiveUpdate()"/></div>
        <div class="bsm-color-row"><label>Text Color</label><input type="color" value="${data.btnTextColor}" oninput="_editingData.btnTextColor=this.value;heroLiveUpdate()"/></div>
      </div>
      ...
      <div class="bsm-field"><label>Text</label><input type="text" value="${data.btn2Text}" oninput="_editingData.btn2Text=this.value;heroLiveUpdate()"/></div>
      <div class="bsm-field"><label>Link</label><input type="url" value="${data.btn2Link}" oninput="_editingData.btn2Link=this.value"/></div>
    </div>
      ...
      <div class="element-color-grid">
        <div class="ecg-item">
          ...
          <div class="ecg-colors">
            <input type="color" title="Color" value="${data.elemHeadingColor||data.textColor}" oninput="_editingData.elemHeadingColor=this.value;heroLiveUpdate()"/>
          </div>
        </div>
        <div class="ecg-item">
          <span class="ecg-label">📝 Subtext</span>
          <div class="ecg-colors">
            <input type="color" title="Color" value="${data.elemSubColor||data.textColor}" oninput="_editingData.elemSubColor=this.value;heroLiveUpdate()"/>
          </div>
        </div>
        <div class="ecg-item">
          <span class="ecg-label">🏷 Badge</span>
          <div class="ecg-colors">
            <input type="color" title="BG" value="${data.elemBadgeBg||'rgba(255,255,255,0.1)'}" oninput="_editingData.elemBadgeBg=this.value;heroLiveUpdate()"/>
            <input type="color" title="Text" value="${data.elemBadgeColor||data.textColor}" oninput="_editingData.elemBadgeColor=this.value;heroLiveUpdate()"/>
          </div>
        </div>
      </div>
    </div>
...
function renderLeadFormForm(data) {
  return `
    ...
    <div id="lf-content-panel" class="bsm-panel active">
      ...
      <div class="bsm-field"><label>Success Message</label><input type="text" value="${data.successMsg||"You're on the list!"}" oninput="_editingData.successMsg=this.value"/></div>
      <div class="bsm-field"><label>Error Message</label><input type="text" value="${data.errorMsg||'Something went wrong. Please try again.'}" oninput="_editingData.errorMsg=this.value"/></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        <div class="bsm-color-row" style="margin:0;"><label>Background</label><input type="color" value="${data.bgColor}" oninput="_editingData.bgColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Text</label><input type="color" value="${data.textColor}" oninput="_editingData.textColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Button</label><input type="color" value="${data.btnColor}" oninput="_editingData.btnColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Accent</label><input type="color" value="${data.accentColor}" oninput="_editingData.accentColor=this.value"/></div>
      </div>
    </div>
        ...
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:12px;margin-bottom:8px;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div class="bsm-field"><label>Type</label>
              <select onchange="_editingData.fields[${i}].type=this.value">
            ...
            <div class="bsm-field"><label>Field Name</label>
              <input type="text" value="${f.name||''}" placeholder="email" oninput="_editingData.fields[${i}].name=this.value"/>
            </div>
          </div>
          <div class="bsm-field"><label>Placeholder</label>
            <input type="text" value="${f.placeholder}" oninput="_editingData.fields[${i}].placeholder=this.value"/>
          </div>
        </div>
      `).join('')}
      ...
      <div class="webhook-section">
        ...
        <div class="bsm-field"><label>Webhook URL</label>
          <input type="url" value="${data.webhookUrl||''}" placeholder="https://hooks.zapier.com/hooks/catch/…"
            oninput="_editingData.webhookUrl=this.value"/>
        </div>
...
function renderPricingForm(data) {
  ...
  const tiersHTML = data.plans.map((p, i) => `
    <div class="pricing-tier-card" id="ptier-${i}">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
        <div class="bsm-field"><label>Tier Name</label><input type="text" value="${p.name}" oninput="_editingData.plans[${i}].name=this.value;document.querySelector('#ptier-${i} .pricing-tier-label').textContent='Tier ${i+1}: '+this.value"/></div>
        <div class="bsm-field"><label>Price</label><input type="text" value="${p.price}" oninput="_editingData.plans[${i}].price=this.value"/></div>
        <div class="bsm-field"><label>Billing Cycle</label><input type="text" value="${p.period}" placeholder="/month" oninput="_editingData.plans[${i}].period=this.value"/></div>
        <div class="bsm-field"><label>Highlight?</label><select onchange="_editingData.plans[${i}].featured=this.value==='true'"><option value="false" ${!p.featured?'selected':''}>No</option><option value="true" ${p.featured?'selected':''}>⭐ Yes</option></select></div>
      </div>
      ...
      <div class="bsm-field"><label>CTA Button Text</label><input type="text" value="${p.ctaText}" oninput="_editingData.plans[${i}].ctaText=this.value"/></div>
      <div class="bsm-field"><label>Features (one per line)</label>
        <textarea rows="4" oninput="_editingData.plans[${i}].features=this.value.split('\n').filter(f=>f.trim())">${p.features.join('\n')}</textarea>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.07);">
        <div class="bsm-color-row" style="margin:0;"><label>Card Color</label><input type="color" value="${p.bgColor||'#1a1a2e'}" oninput="_editingData.plans[${i}].bgColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Border</label><input type="color" value="${p.borderColor||'rgba(255,255,255,0.1)'}" oninput="_editingData.plans[${i}].borderColor=this.value"/></div>
      </div>
    </div>
  `).join('');
  ...
  return `
    ...
    <div id="pricing-section-panel" class="bsm-panel active">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        <div class="bsm-color-row" style="margin:0;"><label>Background</label><input type="color" value="${data.bgColor}" oninput="_editingData.bgColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Text Color</label><input type="color" value="${data.textColor}" oninput="_editingData.textColor=this.value"/></div>
        <div class="bsm-color-row" style="margin:0;"><label>Accent</label><input type="color" value="${data.accentColor}" oninput="_editingData.accentColor=this.value"/></div>
      </div>
    </div>
      ...
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:12px;margin-bottom:8px;">
        ...
        <div class="bsm-field"><label>Image URL</label>
          <input type="url" value="${img.src}" placeholder="https://example.com/image.jpg"
            oninput="_editingData.images[${i}].src=this.value; validateGalleryImg(this, ${i})"/>
...
function buildExportHTML() {
  // Unified renderer: same pipeline as preview (forExport=true strips builder controls).
  // Guarantees pixel-identical export — same CSS vars, renderers, glass effects.
  const siteName = document.getElementById('site-name-input')?.value || 'My Site';
  const previewBase = buildPreviewHTML(true);

  // Splice production metadata into the already-correct preview <head>
  const productionMeta = `  <title>${siteName}</title>
  <meta name="description" content="Built with Supersuite"/>
  ...
  return previewBase.replace('<head>', '<head>\n' + productionMeta);
}
