...
(function SSV27Cleanup(){
  const REMOVED_IDS = [
    ...
    'studio-launch-btn',   // top-bar "🧪 Studio"
    'studio-mode-root',    // entire Studio Mode root
  ...
  function scrub(){
    ...
    document.querySelectorAll('.top-nav button, .nav-right button').forEach(b => {
      ...
      if (txt.startsWith('🧪 Studio')) b.remove();
  ...
  function neutralize(){
    const STUBS = {
      ...
      StudioMode:   ['toggle','setRoute','ensureUI','setDevTab','refreshDev','applyDev','agentSend','agentClear','addApi','updateApi']
    };
