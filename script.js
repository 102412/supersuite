    ...
    order.forEach(type => {
      ...
      if (type === 'features') {
        if (sc) {
          ...
          if (sc.items && data.cards) {
            data.cards = sc.items.map(item => ({ icon: item.icon, title: item.title, body: item.description, bgColor: data.bgColor }));
          }
        } else {
          ...
          if (data.cards) {
            data.cards = [
              { icon:'⚡', title:'Fast & Reliable',  body:'Quick turnaround with consistent, quality results every time.' },
              { icon:'🏆', title:'Proven Expertise',  body:'Years of hands-on experience in ' + industry + ' — local and trusted.' },
              { icon:'💰', title:'Transparent Pricing', body:'Upfront quotes with zero hidden fees. You know what you pay before we start.' },
              { icon:'⭐', title:'5-Star Rated',       body:'Hundreds of happy customers and growing. Check our reviews.' },
              { icon:'📞', title:'Fast Response',      body:'We reply within the hour — no chasing, no waiting.' },
              { icon:'✅', title:'Satisfaction Guarantee', body:'Not happy? We come back and make it right. No questions asked.' },
...
const AIImprover = {
  ...
  _apply(s) {
    ...
    if (s.kind === 'headline' && s.value) {
      ...
      const valid = ['nav','hero','features','leadform','testimonials','pricing','cta','gallery','widget','footer'];
...
const StudioMode = {
  ...
  _agentTools: [
    ...
    { type: 'function', function: { name: 'add_block', description: 'Add a block to the page.', parameters: { type: 'object', required: ['type'], properties: { type: { type: 'string', enum: ['nav','hero','features','leadform','testimonials','pricing','cta','gallery','widget','footer'] } } } } },
...
const StudioAgentApp = {
  ...
  _tools: [
    ...
    { type: 'function', function: { name: 'builder_add_block', description: 'Add a block to the Supersuite builder (requires bridge permission).', parameters: { type: 'object', required: ['type'], properties: { type: { type: 'string', enum: ['nav','hero','features','leadform','testimonials','pricing','cta','gallery','widget','footer'] } } } } },
