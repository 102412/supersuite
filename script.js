...
(function SSV27Cleanup(){
  const REMOVED_IDS = [
    ...
    // 'regen-modal' — kept, used by RegenSystem
      ...
      return `<div class="ss-section-wrapper" data-section-id="${s.id}"
        ...
        <div class="ss-sec-ctrl" style="all:initial;display:none;position:absolute;top:10px;right:10px;z-index:2147483647;flex-direction:row;gap:6px;align-items:center;pointer-events:all;">
          ...
          <button onclick="window.parent.postMessage({type:'moveSection',id:'${s.id}',dir:'up'},'*')"
            style="all:initial;display:inline-block;background:#1a1a2e;color:#fff;border:none;border-radius:7px;padding:7px 10px;font-size:13px;cursor:pointer;line-height:1;box-shadow:0 2px 8px rgba(0,0,0,.4);">↑</button>
          ...
          <button onclick="window.parent.postMessage({type:'deleteSection',id:'${s.id}'},'*')"
            style="all:initial;display:inline-block;background:#ef4444;color:#fff;border:none;border-radius:7px;padding:7px 10px;font-size:13px;cursor:pointer;line-height:1;box-shadow:0 2px 8px rgba(0,0,0,.4);">✕</button>
        </div>
      </div>`;
    }).join('\n');
...
const RegenSystem = (function(){
  ...
  async function run(mode) {
    // Also check SSFreemium gate for free/guest users
    if (typeof SSFreemium !== 'undefined' && (State.userTier === 'free' || !State.userEmail)) {
      if (!SSFreemium.canRegen()) {
        closeModal();
        showToast('⚠️ Regen limit reached', 'Upgrade to continue regenerating → supersuite-checkout.vercel.app', 'error');
        setTimeout(() => {
          const nudge = document.createElement('div');
          nudge.innerHTML = '<div style="position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#111;border:1px solid rgba(249,115,22,.4);border-radius:6px;padding:14px 20px;z-index:9997;display:flex;gap:16px;align-items:center;font-family:\'DM Mono\',monospace;font-size:12px;color:#A3A3A3;white-space:nowrap;animation:ss-nudge-in .3s ease;"><span>Regen limit reached — 3/3 used</span><a href="https://supersuite-checkout.vercel.app?ref=regen" target="_blank" style="background:#F97316;color:#fff;padding:6px 14px;border-radius:4px;text-decoration:none;">Upgrade →</a><button onclick="this.closest(\'div\').parentElement.remove()" style="background:none;border:none;color:#525252;font-size:16px;cursor:pointer;">✕</button></div>';
    ...
    try {
      ...
      if (html && SSections.load(html, fullPrompt)) {
        ...
        if (typeof SSFreemium !== 'undefined') { SSFreemium.useRegen(); SSFreemium.enforcerRegenUI(); }
...
/* ── Parse a section's HTML into editable fields ──────────────── */
...
/* ── Open section editor → now routes to SSModeC ────────────── */
function openSectionEditor(id, clickX, clickY) {
  ...
  const fieldsHTML = fields.length ? `
    <div style="margin-bottom:14px;">
      <label style="font-size:11px;font-weight:700;color:var(--ui-accent);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:10px;">📝 Direct Edit</label>
      ${fields.map((f, i) => `
  ...
  rpContent.innerHTML = `
    <div style="padding:4px 0;">
      ...
      <div style="margin-bottom:14px;">
        <label style="font-size:11px;font-weight:700;color:var(--ui-accent);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:8px;">✨ AI Edit</label>
        <textarea id="sec-ai-input" rows="2"
          placeholder="e.g. make darker, add urgency to headline, change CTA to Book Now…"
        ...
        <button onclick="SectionEditorApply()" id="sec-ai-btn"
          ...
          onmouseover="this.style.background='#e85520'" onmouseout="this.style.background='#ff6b35'">Apply AI Edit →</button>
      </div>

      <hr style="border:none;border-top:1px solid var(--ui-border);margin:12px 0;"/>

      <div style="margin-bottom:12px;">
        <label style="font-size:11px;font-weight:700;color:var(--ui-accent);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:8px;">➕ Add Section</label>
        <input id="sec-add-input" type="text" placeholder="e.g. FAQ, team, contact form…"
        ...
        <button onclick="SectionEditorAdd()"
          style="width:100%;margin-top:7px;padding:9px;background:rgba(255,107,53,0.1);color:#ff6b35;border:1px solid rgba(255,107,53,0.3);border-radius:7px;font-weight:600;font-size:12px;cursor:pointer;font-family:'DM Sans',sans-serif;">＋ Generate &amp; Match Style</button>
      </div>

      <hr style="border:none;border-top:1px solid var(--ui-border);margin:12px 0;"/>

      <div style="display:flex;gap:7px;">
        <button onclick="SSections.move('${id}','up')"   style="flex:1;padding:8px;background:var(--ui-bg2);border:1px solid var(--ui-border);color:var(--ui-text);border-radius:7px;font-size:12px;cursor:pointer;">↑</button>
        <button onclick="SSections.move('${id}','down')" style="flex:1;padding:8px;background:var(--ui-bg2);border:1px solid var(--ui-border);color:var(--ui-text);border-radius:7px;font-size:12px;cursor:pointer;">↓</button>
...
// Direct field edit — replaces text in section HTML instantly
...
/* ══════════════════════════════════════════════════════════════════
   ...
   Flow:
   1. Design system call  → colors, fonts, layout philosophy, sections
   2. Unsplash fetch      → real images matching brand + industry
   3. HTML generation     → AI writes complete custom HTML/CSS
══════════════════════════════════════════════════════════════════ */
...
async function generateCustomSiteHTML(info, _onStep) {
  ...
  // ── Build rich brand brief ─────────────────────────────────────
  const brief = [
    `BUSINESS: ${biz}`,
    `INDUSTRY: ${ind}${niche ? ' — specifically: ' + niche : ''}`,
  ...
  if (_onStep) _onStep(1, 'Planning your design system…');

  // ── Step 1: Design system + section plan ─────────────────────
  const dsRaw = await callAIWorker([
    { role: 'system', content: `You are an elite brand designer. Given a business brief, define a complete unique design system. Make every decision specific to this brand — no generic choices. Output ONLY valid JSON, no markdown:
{
  "primaryColor":     "#hex — main action/brand color tailored to personality",
  "secondaryColor":   "#hex — supporting color",
  "accentColor":      "#hex — highlight/contrast color",
  ...
  "borderRadius":     "e.g. 0px or 4px or 12px — reflects brand personality",
  ...
  if (_onStep) _onStep(2, 'Finding your photos…');

  // ── Step 2: Fetch real Unsplash images ────────────────────────
  ...
  if (_onStep) _onStep(3, 'Writing your website…');

  // ── Step 3: AI writes the complete HTML/CSS from scratch ───────
...
${imageBlock ? 'AVAILABLE UNSPLASH IMAGES (use these exact URLs):\n' + imageBlock : ''}`;
  ...
  if (_onStep) _onStep(4, 'Finalising…');
  ...
  if (info.regenMode === 'design' && info.existingCopy) {
    // The new design has fresh copy from the AI — this is acceptable for design regen
...
function buildPreviewHTML(forExport = false) {
  ...
  const previewInteractScript = !forExport ? `
    <script>
      // ── Block hover controls (blocks-mode specific) ──
      ...
      // ── SHARED CANVAS SCRIPT (inline edit + Mode B/C + action bar) ──
      ...
      (function(){
        ...
        // ── Cursor hint: show text cursor when hovering editable text ──
        ...
        function activate(el, clickEvent) {
          ...
          // Design system styles — use !important to override AI CSS
        ...
        // ── SINGLE CLICK on text element → activate editing ─────────
        document.addEventListener('click', function(e) {
          var t = e.target;
          ...
          if (el) {
            // Text element clicked — activate
            e.stopPropagation(); // prevent Mode B firing
            activate(el, e);
          } else {
            // Clicked outside any text element — save current edit
            if (active) deactivate(active, false);
          }
        }, false);

        // ── Mousedown outside → save immediately (before click fires) ─
      ...
      (function(){
        ...
        // Single click → Mode B only (text handled by inline edit system above)
        document.addEventListener('click', function(e) {
          ...
          // Skip text elements entirely — inline edit system owns those
        ...
        // Block action bar — render above hovered block/section
        ...
        function showActionBar(wrapper) {
          ...
          var btns = [
            {icon:'↑', title:'Move up',    msg:{type:'moveBlockOrSection', id: bid||sid, kind: bid?'block':'section', dir:'up'}},
            {icon:'↓', title:'Move down',  msg:{type:'moveBlockOrSection', id: bid||sid, kind: bid?'block':'section', dir:'down'}},
            {icon:'⧉', title:'Duplicate',  msg:{type:'duplicateBlockOrSection', id: bid||sid, kind: bid?'block':'section'}},
            {icon:'✕', title:'Delete',     msg:{type:'deleteBlockOrSection',   id: bid||sid, kind: bid?'block':'section'}, danger:true},
          ];
      ...
      // ── Shared canvas script: inline editing + Mode B/C + action bar ──
...
/* ══════════════════════════════════════════════════════════════════
   SSAUTH — Simple 2-flow auth (free email/pass · paid via Gumroad)
══════════════════════════════════════════════════════════════════ */
const SSAuth = {
  _busy: false,
  ...
  async go() {
    ...
    // ── Hidden admin / access-code backdoor ───────────────────
      ...
      if (result.error) {
        ...
        if (msg.includes('Invalid login') || msg.includes('invalid_credentials') || msg.includes('Invalid credentials')) {
          ...
          if (!result.access_token) {
            this._reset(btn);
            return this._err('Check your email — click the confirmation link, then sign in.');
          }
          // New account — upsert with free tier
  ...
  forgot() {
    ...
    this._err('✓ If that email exists, a reset link is on its way.');
  },
...
/* ══════════════════════════════════════════════════════════════════
   SSFREEMIUM — localStorage-based usage gate
   Free: 3 regens total · unlimited inline/AI edits · export gates
══════════════════════════════════════════════════════════════════ */
const SSFreemium = (function(){
  const KEY = 'ss_free_session';

  function _load() {
    try { return JSON.parse(localStorage.getItem(KEY) || 'null') || _defaults(); } catch(e) { return _defaults(); }
  }
  function _defaults() { return { regens_used: 0, regen_limit: 3, created_at: Date.now() }; }
  ...
  function useRegen()  { const d = _load(); d.regens_used++; _save(d); }
  function remaining() { const d = _load(); return Math.max(0, d.regen_limit - d.regens_used); }
  function reset()     { _save(_defaults()); }

  function gateExport() {
    window.location.href = 'https://supersuite-checkout.vercel.app?ref=export';
  }

  function enforcerRegenUI() {
    const regenBtn = document.querySelector('.ss-tb-btn[onclick*="RegenSystem"]');
    if (regenBtn && !canRegen()) {
      regenBtn.disabled = true;
      regenBtn.title    = 'Regen limit reached — upgrade to continue';
      regenBtn.style.opacity = '0.4';
      regenBtn.style.cursor  = 'default';
    }
    // Show/hide export sublabel based on tier
    const sub = document.getElementById('ss-export-sublabel');
    if (sub) {
      const isFree = !State.userEmail || State.userTier === 'free' || !State.userTier;
      sub.style.display = isFree ? 'block' : 'none';
    }
  }

  return { canRegen, useRegen, remaining, reset, gateExport, enforcerRegenUI };
})();
window.SSFreemium = SSFreemium;

/* ══════════════════════════════════════════════════════════════════
   SSLANDING — Landing page prompt submission + animations
══════════════════════════════════════════════════════════════════ */
const SSLanding = (function(){
  const PROMPTS = [
    'Build a site for my nail salon in Louisville...',
    'Create a portfolio for my photography studio...',
    'Make a website for my BBQ restaurant in Austin...',
    'Build a landing page for my personal training business...',
    'Create a law firm site for Smith & Associates...',
    'Make a site for my dog grooming salon...',
    'Build a boutique clothing store website...',
  ];

  const CAT_PREVIEWS = {
    websites:    [{ title:'Local Service',  bg:'#f0f0ee', accent:'#1a1a1a' }, { title:'Small Business', bg:'#0a0a0f', accent:'#f97316' }, { title:'Startup',       bg:'#f8f4f0', accent:'#2a2a2a' }],
    portfolios:  [{ title:'Photography',    bg:'#111',    accent:'#fff'     }, { title:'Design Studio', bg:'#faf9f7', accent:'#111'     }, { title:'Architecture',  bg:'#1a1a1a', accent:'#c9b89a' }],
    services:    [{ title:'Plumber',        bg:'#0d1b2a', accent:'#f97316' }, { title:'Dentist',       bg:'#f0f8f4', accent:'#2d7a4f' }, { title:'Lawyer',        bg:'#1a1410', accent:'#c9a84c' }],
    restaurants: [{ title:'Fine Dining',    bg:'#0f0a05', accent:'#d4943a' }, { title:'Casual Bistro', bg:'#faf5ef', accent:'#8b4513' }, { title:'Food Truck',    bg:'#1a0505', accent:'#ff4444' }],
    stores:      [{ title:'Boutique',       bg:'#faf8f5', accent:'#2a2a2a' }, { title:'Tech Shop',     bg:'#0a0a0f', accent:'#4a90d9' }, { title:'Handmade',      bg:'#fdf6ec', accent:'#8b6914' }],
    agencies:    [{ title:'Creative',       bg:'#111',    accent:'#f97316' }, { title:'Digital',       bg:'#0a0f1a', accent:'#4a90d9' }, { title:'Branding',      bg:'#fafaf8', accent:'#2a2a2a' }],
    landing:     [{ title:'SaaS Product',   bg:'#0a0a10', accent:'#6c63ff' }, { title:'App Launch',    bg:'#0f1a0a', accent:'#4caf50' }, { title:'Event',         bg:'#1a0a0a', accent:'#f97316' }],
    personal:    [{ title:'Speaker',        bg:'#111',    accent:'#f5c518' }, { title:'Consultant',    bg:'#faf9f7', accent:'#1a1a1a' }, { title:'Creator',       bg:'#0a0f15', accent:'#e91e8c' }],
  };
  ...
  function init() {
    ...
    document.querySelectorAll('.ss-reveal').forEach(el => io.observe(el));

    // Prompt cycling
    _cycle();

    // Start glow pulse after 2s of idle
    _glowTimer = setTimeout(_startGlow, 2000);

    // Category tabs — render default
    switchCat(document.querySelector('.ss-lp-tab.active'), 'websites');
  }

  function _cycle() {
    const inp = document.getElementById('ss-hero-input');
    if (!inp || !_cycling) return;
    _cycleTimer = setTimeout(() => {
      if (!_cycling) return;
      _promptIdx = (_promptIdx + 1) % PROMPTS.length;
      inp.placeholder = PROMPTS[_promptIdx];
      _cycle();
    }, 3500);
  }

  function _startGlow() {
    const bar = document.getElementById('ss-prompt-bar');
    if (bar && !bar.classList.contains('ss-glow-idle')) bar.classList.add('ss-glow-idle');
  }
  ...
  function onInput() {
    ...
    const bar = document.getElementById('ss-prompt-bar');
    if (bar) bar.classList.remove('ss-glow-idle');
  }
  ...
  function useChip(el) {
    ...
    inp.value = 'Build a site for my ' + text.toLowerCase();
    inp.focus();
    onInput();
  }

  function switchCat(tab, catId) {
    document.querySelectorAll('.ss-lp-tab').forEach(t => t.classList.remove('active'));
    if (tab) tab.classList.add('active');
    const container = document.getElementById('ss-cat-previews');
    if (!container) return;
    const cards = (CAT_PREVIEWS[catId] || CAT_PREVIEWS.websites);
    container.style.opacity = '0';
    setTimeout(() => {
      container.innerHTML = cards.map(c => `
        <div class="ss-lp-preview-card" onclick="document.getElementById('ss-hero-input').value='Build a ${c.title.toLowerCase()} website';SSLanding.onInput();document.getElementById('ss-hero').scrollIntoView({behavior:'smooth'})">
          <div class="ss-lp-preview-thumb" style="background:${c.bg};">
            <div style="position:absolute;inset:0;display:flex;align-items:flex-end;padding:12px;">
              <div style="width:100%;">
                <div style="height:8px;background:${c.accent};border-radius:2px;width:55%;margin-bottom:6px;opacity:0.9;"></div>
                <div style="height:4px;background:${c.accent};border-radius:2px;width:70%;opacity:0.35;"></div>
                <div style="height:4px;background:${c.accent};border-radius:2px;width:40%;margin-top:3px;opacity:0.25;"></div>
              </div>
            </div>
          </div>
          <div class="ss-lp-preview-info"><div class="ss-lp-preview-title">${c.title}</div></div>
        </div>`).join('');
  ...
  async function submit() {
    const inp  = document.getElementById('ss-hero-input');
    const btn  = document.getElementById('ss-prompt-btn');
    let text   = (inp?.value || '').trim();
    // Use placeholder if empty so the button always triggers generation
    if (!text) {
      const placeholders = ['a nail salon in New York', 'a photography studio', 'a BBQ restaurant', 'a law firm', 'a personal trainer'];
      text = 'Build a site for ' + placeholders[Math.floor(Math.random() * placeholders.length)];
      if (inp) inp.value = text;
    }

    _cycling = false;
    if (btn) { btn.disabled = true; document.getElementById('ss-prompt-btn-text').textContent = 'Building…'; }

    // Show loading screen
    const lp      = document.getElementById('landing-page');
    const loading = document.getElementById('ss-loading');
    if (lp)      { lp.style.transition = 'opacity 400ms ease'; lp.style.opacity = '0'; }
    if (loading) { loading.style.display = 'flex'; loading.style.opacity = '0'; loading.style.transition = 'opacity 300ms ease'; }
    setTimeout(() => { if (loading) loading.style.opacity = '1'; if (lp) lp.style.display = 'none'; }, 400);

    // Cycle loading messages
    const msgs = ['Analyzing your business…', 'Designing your layout…', 'Writing your content…', 'Generating your site…', 'Pulling photography…', 'Almost ready…'];
    let msgIdx = 0;
    const msgEl = document.getElementById('ss-loading-msg');
    const barEl = document.getElementById('ss-loading-bar');
    const msgInterval = setInterval(() => {
      msgIdx = Math.min(msgIdx + 1, msgs.length - 1);
      if (msgEl) { msgEl.style.opacity='0'; setTimeout(()=>{ msgEl.textContent = msgs[msgIdx]; msgEl.style.opacity='1'; }, 150); }
      if (barEl) barEl.style.width = ((msgIdx / msgs.length) * 90) + '%';
    }, 1800);
    ...
    try {
      ...
      // Activate a guest session
      State.authenticated = true;
      State.userTier      = 'free';
      State.userEmail     = '';
      State.businessName  = info.businessName;
      State.industry      = info.industry;
      SSFreemium.enforcerRegenUI();

      // Generate the site
      const html = await generateCustomSiteHTML(info);
      if (!html || html.length < 400) throw new Error('Empty generation');
      ...
      setTimeout(() => {
        ...
        // initApp but skip onboarding since we already have sections
        _launchBuilder();
      }, 600);

    } catch(err) {
      clearInterval(msgInterval);
      console.error('[SSLanding.submit]', err);
      if (loading) { loading.style.display = 'none'; }
      if (lp) { lp.style.display = ''; lp.style.opacity = '0'; lp.style.transition = 'opacity 300ms ease'; setTimeout(() => { lp.style.opacity = '1'; }, 50); }
      if (btn) { btn.disabled = false; document.getElementById('ss-prompt-btn-text').textContent = 'Build My Site'; }
      showToast('⚠️ Generation failed', 'Could not generate site. Please try again.', 'error');
    }
  }

  function _parsePrompt(text) {
    // Extract business type and name from prompt
    const lower = text.toLowerCase();
    const industries = {
      'nail salon':'nail salon', 'salon':'beauty salon', 'restaurant':'restaurant',
      'photographer':'photography', 'photography':'photography', 'plumber':'plumbing',
      'lawyer':'law firm', 'law firm':'law firm', 'dentist':'dental',
      'gym':'fitness', 'personal trainer':'fitness', 'boutique':'retail',
      'bakery':'bakery', 'coffee':'coffee shop', 'agency':'agency',
      'real estate':'real estate', 'landscaping':'landscaping', 'cleaning':'cleaning service',
    };
    let industry = 'service business';
    for (const [kw, ind] of Object.entries(industries)) {
      if (lower.includes(kw)) { industry = ind; break; }
    }
    // Try to extract business name
    const nameMatch = text.match(/for\s+(my\s+)?([A-Z][^,\.]+)/);
    const bizName = nameMatch ? nameMatch[2].trim() : 'Your Business';
    return {
  ...
  function _launchBuilder() {
    // Wire up tier badge
    const tierBadge = document.getElementById('nav-tier-badge');
    if (tierBadge) { tierBadge.textContent = 'Free'; tierBadge.className = 'nav-tier-badge free'; }
    // Wire UsageTracker if available
    if (typeof UsageTracker !== 'undefined') UsageTracker.init('free');
    // Wire up AIHub
    setTimeout(() => { try { AIHub.mount(); } catch(e){} }, 800);
    // Show remaining regens
    const rem = SSFreemium.remaining();
    showToast('✨ Site ready', `${rem} regen${rem!==1?'s':''} remaining on free plan`, 'success');
    // Enforce regen UI state
    SSFreemium.enforcerRegenUI();
    // Update layers/sections panel
    SSections.updatePanel();
    SSPanels && SSPanels.closeAll && SSPanels.closeAll();
  }

  return { init, useChip, submit, onInput, switchCat };
})();
window.SSLanding = SSLanding;

// Auto-init landing page when DOM ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => { if (document.getElementById('ss-hero')) SSLanding.init(); });
} else {
  if (document.getElementById('ss-hero')) SSLanding.init();
}
...
/* ═══════════════════════════════════════════════════════════════════
   BUILDER V2 — LAYOUT SYSTEM
   ─────────────────────────────────────────────────────────────────
   MIGRATION MAP:
     Sections tab     → SSPanels.toggle('sections')
     Projects tab     → SSPanels.toggle('projects')
     Layers tab       → SSPanels.toggle('layers')
     Templates tab    → DROPPED
     Global Styles    → DROPPED
     Right panel      → Mode C (double-click → SSModeC)
     Canvas toolbar   → Top bar undo/redo buttons
     Block settings   → Mode C floating block editor (SSModeC)
   NEW:
     Connectors       → SSPanels.toggle('connectors') → SSConnectors
     Mode A (click text)       → inline text edit (already in iframe)
     Mode B (click bg)         → SSModeB floating color/style panel
     Mode C (double-click blk) → SSModeC floating block editor
     Block action bar          → rendered inside iframe
═══════════════════════════════════════════════════════════════════ */

/* ── SSPanels — top bar floating panel manager ───────────────── */
...
/* ── SSProjectPanel — renders the projects floating panel ─────── */
  ...
  render() {
    ...
    list.innerHTML = projects.map((p, i) => `
      <div class="ss-project-card ${p.active ? 'active' : ''}" onclick="loadProject(${i});SSPanels.close('projects')">
        ...
        <button onclick="event.stopPropagation();deleteProject(${i})" style="background:none;border:none;color:var(--ss-text-3);cursor:pointer;font-size:12px;padding:0;" title="Delete">✕</button>
      </div>`).join('');
  }
};
window.SSProjectPanel = SSProjectPanel;

/* ── Mode B — background click color/style panel ─────────────── */
...
/* ── Mode C — double-click block editor panel ────────────────── */
const SSModeC = (function(){
  ...
  function _buildSectionEditor(sec) {
    ...
    return `
      ...
      <div class="ss-panel-field">
        <textarea id="sec-ai-input" class="ss-panel-input ss-panel-textarea" rows="2" placeholder="Tell AI what to change…"
          onkeydown="if(event.key==='Enter'&&(event.metaKey||event.ctrlKey))SectionEditorApply()"></textarea>
        <button class="ss-panel-btn ss-panel-btn-primary" style="margin-top:6px;width:100%;" onclick="SectionEditorApply()" id="sec-ai-btn">Apply AI Edit →</button>
      </div>
      <div class="ss-be-section-title">Section</div>
      <div style="display:flex;gap:6px;">
        <button class="ss-panel-btn" style="flex:1;" onclick="SSections.move('${sec.id}','up');SSModeC.refresh()">↑</button>
        <button class="ss-panel-btn" style="flex:1;" onclick="SSections.move('${sec.id}','down');SSModeC.refresh()">↓</button>
      ...
      <div class="ss-panel-field">
        <input type="text" id="sec-add-input" class="ss-panel-input" placeholder="e.g. FAQ, team, contact form…"
          onkeydown="if(event.key==='Enter')SectionEditorAdd()"/>
        <button class="ss-panel-btn" style="margin-top:6px;width:100%;" onclick="SectionEditorAdd()">＋ Generate & Match Style</button>
      </div>`;
  }
  ...
  function saveBlock() {
    if (_activeType === 'block' && _activeId && _editingData) {
      ...
      if (block) {
        ...
        refreshPreview();
        updateLayers();
        showToast('✅ Saved', 'Block updated', 'success');
        saveToSession();
      }
    }
    close();
...
/* ── SSConnectors — 5-connector integration system ───────────── */
const SSConnectors = (function(){
    ...
    content.innerHTML = `
      ...
      ${c.authType === 'oauth' ? `
        ...
        <button class="ss-panel-btn ss-panel-btn-primary" style="width:100%;padding:10px;" onclick="SSConnectors.startOAuth('${c.id}')">
          Continue with ${c.name.includes('Google') ? 'Google' : c.name} →
        </button>
      ` : `
        <div class="ss-panel-field">
  ...
  function saveApiKey(id, storeKey) {
    ...
    if (!val) { showToast('⚠️ Required', 'Enter a value', 'error'); return; }
    const data = {}; data[storeKey] = val;
    _set(KEYS[id], data);
    closeAuth();
    render();
    showToast('✅ Connected', CONNECTORS.find(c=>c.id===id)?.name + ' connected', 'success');
  }
...
function applyInlineEdit(msg) {
  ...
  // ── New: ID-based approach (reliable for all element types) ──
  if (eid) {
    if (sectionId) {
      const sec = State.sections.find(s => s.id === sectionId);
      if (!sec) return;
      try {
        ...
        if (el) {
          ...
          History.push();
          saveToSession();
          // No refreshPreview() — DOM is already showing the right state.
    ...
    if (blockId) {
      // Blocks store structured data — extract plain text from the new HTML
  ...
  // ── Legacy text-match fallback (used when eid not present) ────
...
/* ── Shared canvas interaction script ───────────────────────────
...
/* ══ INLINE TEXT EDITING — Canva/Slides style ══════════════════ */
var _active    = null;
...
/* SINGLE CLICK — activate text editing */
document.addEventListener('click', function(e){
...
/* mousedown outside → save before click fires */
document.addEventListener('mousedown', function(e){
  if (!_active) return;
...
/* ══ MODE B (background click) + MODE C (double-click) ════════ */
var TEXT_SET = {};
...
function _showBar(w) {
  ...
  var defs=[
    {i:'↑',t:'Up',   m:{type:'moveBlockOrSection',    id:bid||sid,kind:bid?'block':'section',dir:'up'}},
    {i:'↓',t:'Down', m:{type:'moveBlockOrSection',    id:bid||sid,kind:bid?'block':'section',dir:'down'}},
    {i:'⧉',t:'Dup',  m:{type:'duplicateBlockOrSection',id:bid||sid,kind:bid?'block':'section'}},
    {i:'✕',t:'Del',  m:{type:'deleteBlockOrSection',   id:bid||sid,kind:bid?'block':'section'},d:true},
  ];
...
function _replaceTextNodes(html, from, to) {
  try {
    ...
    walk(body);
    // Serialise — use innerHTML of the body wrapper
...
────────────────────────────────────────────────────────────────── */
function exportSite() {
  // Free tier: redirect to checkout instead of exporting
  const isFreeTier = !State.userEmail || State.userTier === 'free' || State.userTier === '';
  if (isFreeTier && typeof SSFreemium !== 'undefined') {
    SSFreemium.gateExport();
    return;
  }
...
────────────────────────────────────────────────────────────────── */
/* ══════════════════════════════════════════════════════════════════
   USAGE LIMITER — weekly quotas per tier
   free:   1 site/wk · 2 regens/wk · 20 AI edits/wk
   single: 5 sites  · 5 regens   · 100 AI edits
   pro:    ∞ sites  · 8 regens   · ∞  AI edits
   agency: ∞ all
══════════════════════════════════════════════════════════════════ */
const UsageLimiter = (function(){
  ...
  function showUpgradePrompt(type) {
    ...
    showToast('⚠️ ' + (msgs[type]||'Limit reached'), 'Upgrade for more → supersuite-checkout.vercel.app', 'error');
    setTimeout(() => {
      ...
      el.innerHTML = `
        <div style="position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#111118;border:1px solid rgba(255,107,53,0.4);border-radius:14px;padding:16px 24px;z-index:9997;display:flex;align-items:center;gap:16px;box-shadow:0 20px 60px rgba(0,0,0,.6);font-family:'DM Sans',sans-serif;animation:ss-nudge-in .3s ease;">
          <div>
            ...
            <div style="font-size:12px;color:rgba(255,255,255,0.45);">Resets next week · Upgrade for unlimited</div>
          </div>
          <a href="https://supersuite-checkout.vercel.app" target="_blank" style="background:#ff6b35;color:#fff;padding:9px 18px;border-radius:8px;font-weight:700;font-size:13px;text-decoration:none;white-space:nowrap;flex-shrink:0;">Upgrade →</a>
          <button onclick="this.closest('#ss-upgrade-nudge').remove()" style="background:none;border:none;color:rgba(255,255,255,0.3);font-size:18px;cursor:pointer;padding:4px;">✕</button>
        </div>`;
...
const SS_SITE_KEY = 'ss_site_v1'; // localStorage — survives session wipes
...
Make each design VISUALLY DISTINCT. Use different fonts, colors, border-radius, and layouts. The 3 designs should look nothing alike.`;
  ...
  /* ── BRAND INTERVIEW — 12-question conversational session ────── */
  ...
  _renderStep() {
    ...
    const GOALS = [
      { id:'get-leads',     label:'Get leads',          icon:'📥' },
      { id:'sell-products', label:'Sell products',       icon:'🛒' },
      { id:'credibility',   label:'Build credibility',   icon:'⭐' },
      { id:'book-appts',    label:'Book appointments',   icon:'📅' },
      { id:'showcase-work', label:'Showcase portfolio',  icon:'🖼' },
      { id:'inform',        label:'Inform & educate',    icon:'📖' },
    ];
    ...
    const stepContent = () => {
      switch(step) {
        case 0: return `
          ...
          <div class="ob-field"><label class="ob-label">What do you do? <span class="ob-opt">(optional — more detail = better site)</span></label>
        ...
        case 1: return `
          ...
          <input id="ob-location" class="ob-input" type="text" placeholder="e.g. East London · Toronto · Miami Beach" value="${this._esc(d.location)}"/></div>`;

        case 2: return `
          <div class="ob-q-intro">Let's capture the personality of ${biz}.</div>
          <h2 class="ob-title">Pick 3–5 words that describe your brand vibe.</h2>
          ...
          <div class="ob-chip-hint">${d.vibes.length ? d.vibes.join(' · ') : 'Select 3–5 words'}</div>`;
        ...
        case 3: return `
          ...
          <textarea id="ob-customer" class="ob-textarea" rows="4" placeholder="e.g. Women aged 25–40, professional, treat themselves monthly. They care about quality and Instagram-worthy results more than price. They book 2 weeks in advance and tip well.">${this._esc(d.targetCustomer)}</textarea>`;
        ...
        case 4: return `
          ...
          <div class="ob-field"><label class="ob-label">Competitor sites <span class="ob-opt">(paste 1–2 URLs)</span></label>
          ...
          <div class="ob-field"><label class="ob-label">Aesthetic inspiration <span class="ob-opt">(any industry, 1–2 URLs)</span></label>
        ...
        case 8: return `
          ...
          <div class="ob-field"><textarea id="ob-colorprefs" class="ob-textarea" rows="2" placeholder="e.g. Primary: #C2A876 (champagne gold) · Secondary: #1A1A1A (near black)&#10;Or: warm earthy tones, terracotta and cream&#10;Or: leave blank — AI decides">${this._esc(d.colorPrefs)}</textarea></div>`;

        case 9: return `
          <div class="ob-q-intro">A few practical details.</div>
          <h2 class="ob-title">What are your core services or products?</h2>
          <div class="ob-field"><textarea id="ob-services" class="ob-textarea" rows="3" placeholder="e.g. Gel extensions £65 · Acrylic set £55 · Nail art from £75 · Remove & reshape £40">${this._esc(d.services)}</textarea></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:6px;">
            ...
            <input id="ob-cta" class="ob-input" type="text" placeholder="Book Now · Get a Quote" value="${this._esc(d.cta)}"/></div>
          </div>`;
        ...
        case 10: return `
          ...
          <div class="ob-field"><textarea id="ob-sections" class="ob-textarea" rows="2" placeholder="e.g. Menu / price list · Team page · Before & after gallery · FAQ · Booking form · Portfolio · Testimonials">${this._esc(d.sections)}</textarea></div>
          <div class="ob-field" style="margin-top:14px;">
            <label class="ob-label">Logo <span class="ob-opt">(optional — AI creates a text logotype if none)</span></label>
            <div class="ob-logo-drop" id="ob-logo-drop" onclick="document.getElementById('ob-logo-file').click()" style="${d.logo?'border-color:rgba(255,107,53,.5)':''}">
              ${d.logo
                ? `<img src="${d.logo}" style="max-height:60px;max-width:180px;object-fit:contain;" alt="logo"/><div class="ob-logo-sub" style="margin-top:6px;color:rgba(255,107,53,.8);">✓ Logo uploaded</div>`
                : `<div class="ob-logo-icon">📎</div><div class="ob-logo-text">Click to upload logo</div><div class="ob-logo-sub">PNG, SVG, JPG</div>`}
        ...
        case 11: return `
          ...
          <textarea id="ob-extra" class="ob-textarea" rows="4" placeholder="e.g. The vibe should be sophisticated but not cold — we want women to feel pampered and welcome, not intimidated. Avoid anything too trendy or Gen Z. We don't want cursive fonts. The hero should feel like walking into a high-end spa.">${this._esc(d.extra)}</textarea>
          <p class="ob-sub" style="margin-top:10px;font-size:11px;opacity:.5;">After this — your site will be generated completely from scratch. Every pixel unique to ${biz}.</p>`;
    ...
    foot.innerHTML = `
      <div class="ob-foot-left">
        ${step > 0 ? '<button type="button" class="ob-btn ob-btn-ghost" onclick="Onboarding.back()">← Back</button>' : ''}
    ...
    /* OLD 3-STEP CONTENT REMOVED — replaced by 12-step interview above */
    if (false) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 1 OF 3 · THE BASICS</div>
        <h2 class="ob-title">Tell us about your business</h2>
        <p class="ob-sub">Just a name and what you do — AI handles everything else.</p>
        ...
        <label class="ob-field">
          <span class="ob-label">WHAT DO YOU DO? <span style="opacity:.5;font-weight:400">optional — more detail = better site</span></span>
        ...
        <label class="ob-field">
          ...
          <input type="text" id="ob-location" class="ob-input" placeholder="e.g. Toronto · East London · Miami Beach" value="${this._esc(d.location)}">
        </label>`;

    // ── Step 1: Brand feel (all optional) ────────────────────────
    } else if (this._step === 1) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 2 OF 3 · YOUR BRAND <span style="opacity:.5;font-weight:400">— all optional</span></div>
        <h2 class="ob-title">How should your site feel?</h2>
        <p class="ob-sub">Skip any of these — they just make the result more you.</p>
        <label class="ob-field">
          <span class="ob-label">BRAND PERSONALITY — pick up to 3</span>
          <div class="ob-chip-grid" style="margin-top:8px;">
            ${this.VIBES.map(v => `<button type="button" class="ob-chip ${d.vibes.includes(v)?'selected':''}" onclick="Onboarding._toggleVibe('${v}')">${v}</button>`).join('')}
          </div>
          ${d.vibes.length ? `<div class="ob-chip-hint" style="margin-top:6px;">${d.vibes.join(' · ')}</div>` : ''}
        </label>
        <label class="ob-field">
    ...
    // ── Step 2: Details + generate (all optional) ────────────────
    } else if (this._step === 2) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 3 OF 3 · FINAL DETAILS <span style="opacity:.5;font-weight:400">— all optional</span></div>
        ...
        <label class="ob-field">
          ...
          <textarea id="ob-services" class="ob-textarea" rows="2" placeholder="e.g. Emergency callouts · drain unblocking · boiler installs">${this._esc(d.services)}</textarea>
        </label>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ...
        <label class="ob-field">
          ...
          <textarea id="ob-extra" class="ob-textarea" rows="2" placeholder="e.g. Dark premium feel. No stock photo suits. Avoid clichés.">${this._esc(d.extra)}</textarea>
        </label>`;
    }
  ...
  next() {
    ...
    // Collect current step's fields — each case in {} to avoid const scope issues
    switch(step) {
      case 0: {
        const name = g('ob-biz-name');
        if (!name) { this._shake(); showToast('⚠️ We need your business name', '', 'warning'); return; }
        d.businessName = name;
  ...
  /* ── Generation — called at end of interview ─────────────────── */
  async _runGeneration() {
    ...
    const m = document.getElementById('onboarding-modal');
    if (!m) { showToast('⚠️ Modal not found', 'Try refreshing the page.', 'error'); return; }

    m.style.display = 'flex';
    m.innerHTML = `
      <div class="ob-card" style="text-align:center;padding:52px 36px;min-width:320px;">
        <div style="font-size:52px;margin-bottom:18px;">🤖</div>
        <h2 style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:#fff;margin-bottom:8px;">Building your site…</h2>
        ...
        <div style="display:flex;flex-direction:column;gap:10px;max-width:260px;margin:0 auto 24px;" id="ob-gen-steps">
          <div id="ogstep-1" style="font-size:13px;color:#ff6b35;font-weight:600;font-family:'DM Sans',sans-serif;">✦ Understanding your brand…</div>
          <div id="ogstep-2" style="font-size:13px;color:rgba(255,255,255,0.25);font-family:'DM Sans',sans-serif;">✦ Writing your copy</div>
          <div id="ogstep-3" style="font-size:13px;color:rgba(255,255,255,0.25);font-family:'DM Sans',sans-serif;">✦ Choosing your design</div>
          <div id="ogstep-4" style="font-size:13px;color:rgba(255,255,255,0.25);font-family:'DM Sans',sans-serif;">✦ Building your sections</div>
        </div>
    ...
    const _tick = (n, msg) => {
      for (let i=1;i<=4;i++) {
        ...
        if (!el) continue;
        if (i < n)  { el.style.color='rgba(255,255,255,0.5)'; el.textContent = '✓ ' + el.textContent.replace(/^[✦✓] /,''); }
        if (i === n){ el.style.color='#ff6b35'; el.style.fontWeight='600'; if(msg) el.textContent = '✦ ' + msg; }
    ...
    const _showError = (msg) => {
      ...
      if (el) { el.style.display='block'; el.textContent = '⚠️ ' + msg + ' — click below to try again.'; }
      // Show retry button
    ...
    try {
      _tick(1, 'Understanding your brand…');
      ...
      showToast('✅ ' + (d.businessName || 'Your site') + ' is ready', 'Double-click any text to edit it inline.', 'success');
  ...
  /* finish() — kept for backward compat with existing patches */
...
/* patchOnboarding — superseded by Onboarding._runGeneration. */
(function patchOnboarding(){
  return; // disabled — _runGeneration handles generation directly
...
/* SSV28+ — finish() is now defined directly on Onboarding._runGeneration.
   This patch is kept for backward compat but delegates to _runGeneration. */
// patchOnboardingV264 — superseded by Onboarding._runGeneration defined above.
    ...
    try {
      if (this._mode === 'signup') {
        ...
        if (!result.access_token && !result.session?.access_token) {
          if (btn) { btn.textContent = 'Sign Up →'; btn.disabled = false; }
          return this._fail('Check your email — we sent a confirmation link. Click it then sign in here.');
        }
...
const AIHub = {
  ...
  mount() {
    ...
    fab.title     = 'AI Editor — drag to move';
    ...
    panel.innerHTML = `
      <div class="aihub-head">
        <div>
          <div class="aihub-eyebrow">AI EDITOR · Groq Llama 3.3</div>
      ...
      <div class="aihub-input-area">
        ...
        <div class="aihub-foot-row">
          <button class="aihub-undo" onclick="AIHub._undo()">↩ Undo</button>
          <span class="aihub-model">Groq · Llama 3.3 70B</span>
        </div>
      </div>`;
  ...
  _undo() {
    ...
    try {
      ...
      refreshPreview();
      SSections.updatePanel();
      this._addMessage('assistant', 'Reverted ✓');
...
Business: ${State.businessName || 'Unknown'} · ${State.industry || ''}
...
{
  "reply": "one sentence — what you did",
  "actions": [
...
Use only the action types needed. Actions run in order. Use real section IDs from the list.`;
      ...
      this._addMessage('assistant', '⚠️ ' + this._esc(err.message || 'Something went wrong. Try again.'));
    } finally {
    ...
    switch(act.type) {
      ...
      case 'edit_section':
        if (act.sectionId && act.instruction) {
          ...
          this._addMessage('assistant', '⏳ Editing ' + (State.sections.find(s=>s.id===act.sectionId)?.label||'section') + '…');
          await SSections.editWithAI(act.sectionId, act.instruction);
          // Remove the "editing…" message
          const msgs = document.getElementById('aihub-messages');
      ...
      case 'add_section':
        if (act.description) {
          this._addMessage('assistant', '⏳ Building: ' + act.description + '…');
