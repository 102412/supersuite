...
const SS_PASSWORD_DB = {
  ...
  agency: { SSAGENCY: { note: 'Pro plan' }, poppy: { note: 'Dev access' } },
};
...
const BlockDefs = {
  nav: {
    ...
    defaultData: { logo:'Supersuite', links:['Home','Features','Pricing','Contact'], bgColor:'#ffffff', textColor:'#1a1a2e', sticky:true, ctaText:'Get Started', ctaLink:'#cta', ctaBgColor:'#ff6b35', ctaTextColor:'#ffffff' }
  },
  hero: {
...
const BlockRenderers = {
  ...
  nav(data) {
    ...
    const tc       = data.textColor  || '#1a1a2e';
    const ctaBg    = data.ctaBgColor || '#ff6b35';
    const ctaColor = data.ctaTextColor || '#ffffff';
    const logo     = data.logo       || 'Site';
    const ctaText  = data.ctaText    || 'Get Started';
    const ctaLink  = data.ctaLink    || '#';
    const sticky   = data.sticky !== false;
    const links    = (data.links||[]).map(l=>`<a href="#" style="color:${tc};text-decoration:none;font-size:15px;font-weight:500;opacity:0.8;transition:opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'">${l}</a>`).join('');
    return `
<nav class="ss-block ss-nav" style="background:${bg};color:${tc};position:${sticky?'sticky':'relative'};top:0;z-index:100;border-bottom:1px solid rgba(0,0,0,0.06);padding:0 var(--section-pad);">
  <div class="ss-nav-inner" style="max-width:var(--container);margin:0 auto;display:flex;align-items:center;justify-content:space-between;height:72px;gap:32px;">
    <div class="ss-nav-logo" style="font-family:var(--font-heading);font-weight:800;font-size:24px;color:${tc};letter-spacing:-0.5px;">${logo}</div>
    <div class="ss-nav-links" style="display:flex;align-items:center;gap:32px;">${links}</div>
    <a href="${ctaLink}" style="background:${ctaBg};color:${ctaColor};padding:10px 22px;border-radius:var(--btn-radius);font-size:14px;font-weight:600;text-decoration:none;transition:all 0.2s;display:inline-flex;align-items:center;gap:6px;" onmouseover="this.style.opacity='0.9';this.style.transform='translateY(-1px)'" onmouseout="this.style.opacity='1';this.style.transform='translateY(0)'">${ctaText} →</a>
  </div>
</nav>`;
  },
  ...
  const blocksHTML = State.blocks.map(block => {
    ...
    return `<div class="ss-block-wrapper" data-block-id="${block.id}" style="position:relative;">
      ...
      <div class="ss-block-controls" style="position:absolute;top:8px;right:8px;display:none;z-index:200;gap:4px;flex-wrap:nowrap;">
        <button onclick="window.parent.postMessage({type:'openBlockSettings',id:'${block.id}'},'*')" style="background:#1a1a2e;color:white;border:none;border-radius:6px;padding:6px 10px;font-size:11px;font-weight:600;cursor:pointer;font-family:DM Sans,sans-serif;">✏️ Edit</button>
  ...
  const empty = State.blocks.length===0&&!forExport ? `
    <div style="min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;background:#f8f8ff;font-family:DM Sans,sans-serif;">
      ...
      <div style="display:flex;gap:10px;margin-top:8px;flex-wrap:wrap;justify-content:center;">
        <button onclick="window.parent.postMessage({type:'addBlock',blockType:'hero'},'*')" style="background:#ff6b35;color:white;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:600;cursor:pointer;font-family:DM Sans,sans-serif;">+ Add Hero</button>
        <button onclick="window.parent.postMessage({type:'addBlock',blockType:'nav'},'*')" style="background:#1a1a2e;color:white;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:600;cursor:pointer;font-family:DM Sans,sans-serif;">+ Add Nav</button>
        <button onclick="window.parent.postMessage({type:'addBlock',blockType:'features'},'*')" style="background:#1a1a2e;color:white;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:600;cursor:pointer;font-family:DM Sans,sans-serif;">+ Add Features</button>
      </div>
    </div>` : '';
...
function renderNavForm(data) {
  ...
  const tc    = data.textColor  || '#1a1a2e';
  const ctaBg = data.ctaBgColor || '#ff6b35';
  const ctaTc = data.ctaTextColor || '#ffffff';
  return `
    ...
    <div class="bsm-field"><label>CTA Button Text</label><input type="text" value="${data.ctaText||'Get Started'}" oninput="_editingData.ctaText=this.value"/></div>
    ...
    <div class="bsm-field"><label>Background Color</label><div class="bsm-color-row"><input type="color" value="${bg}" oninput="_editingData.bgColor=this.value;this.nextElementSibling.value=this.value"/><input type="text" value="${bg}" oninput="_editingData.bgColor=this.value;this.previousElementSibling.value=this.value"/></div></div>
    <div class="bsm-field"><label>Text / Links Color</label><div class="bsm-color-row"><input type="color" value="${tc}" oninput="_editingData.textColor=this.value;this.nextElementSibling.value=this.value"/><input type="text" value="${tc}" oninput="_editingData.textColor=this.value;this.previousElementSibling.value=this.value"/></div></div>
    <div class="bsm-field"><label>CTA Button Background</label><div class="bsm-color-row"><input type="color" value="${ctaBg}" oninput="_editingData.ctaBgColor=this.value;this.nextElementSibling.value=this.value"/><input type="text" value="${ctaBg}" oninput="_editingData.ctaBgColor=this.value;this.previousElementSibling.value=this.value"/></div></div>
    <div class="bsm-field"><label>CTA Button Text Color</label><div class="bsm-color-row"><input type="color" value="${ctaTc}" oninput="_editingData.ctaTextColor=this.value;this.nextElementSibling.value=this.value"/><input type="text" value="${ctaTc}" oninput="_editingData.ctaTextColor=this.value;this.previousElementSibling.value=this.value"/></div></div>
    <div class="bsm-field"><label>Sticky Navigation</label><select onchange="_editingData.sticky=this.value==='true'"><option value="true" ${data.sticky!==false?'selected':''}>Yes — stick to top</option><option value="false" ${data.sticky===false?'selected':''}>No — static</option></select></div>`;
}
...
/* postMessage bridge — handles messages from the sandboxed preview iframe */
