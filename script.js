...
const LiveSync = {
  ...
  init(code, isOwner) {
    ...
    try {
      ...
      console.warn('BroadcastChannel not supported, collab disabled');
      return;
    }
    this.channel.onmessage = (ev) => this._onMessage(ev.data);
    this._updateUI(true);
    console.log('[LiveSync] Session started:', code, isOwner ? '(owner)' : '(collab)');
  },
  ...
  /** Broadcast current full state to all peers */
  push() {
    if (!this.channel || this._syncInProgress) return;
    this._stateVersion++;
    const payload = {
      type: 'STATE_UPDATE',
      version: this._stateVersion,
      ts: Date.now(),
      sender: this.isOwner ? 'owner' : 'collab',
      state: {
        blocks: State.blocks,
  ...
  _onMessage(data) {
    ...
    // Last-write-wins: ignore older versions
    if (data.version <= this._lastRemoteVersion) return;
    this._lastRemoteVersion = data.version;

    this._syncInProgress = true;
    try {
      State.blocks = data.state.blocks || [];
      State.globalStyles = { ...State.globalStyles, ...data.state.globalStyles };
      State.currentTemplate = data.state.currentTemplate || State.currentTemplate;
      State.customCSS = data.state.customCSS || '';
      State.blockIdCounter = data.state.blockIdCounter || State.blockIdCounter;
      refreshPreview();
      updateLayers();
  ...
  _flashSyncIndicator(state) {
    ...
    bar.className = 'collab-sync-bar ' + (state === 'syncing' ? 'syncing' : state === 'error' ? 'error' : '');
    status.textContent = state === 'syncing' ? 'Syncing…' : state === 'error' ? 'Sync error' : 'Synced';
    if (state !== 'syncing') {
      setTimeout(() => {
  ...
  _updateUI(active) {
    ...
    if (info && active) {
      info.textContent = this.isOwner
        ? 'You are the owner · Code: ' + this.sessionCode
        : 'Connected as collaborator · Code: ' + this.sessionCode;
    }
  },

  /** Call after any state-mutating action */
  notifyChange() {
    if (this.channel && !this._syncInProgress) {
      // Debounce: only push after 120ms of silence
      clearTimeout(this._pushTimer);
      this._pushTimer = setTimeout(() => this.push(), 120);
    }
  },
};
...
function buildPreviewHTML(forExport = false) {
  ...
  const blocksHTML = State.blocks.map(block => {
    ...
    if (!forExport && block.data.glassTint && block.data.glassTint !== 'none') {
...
function refreshPreview() {
  ...
  frame.srcdoc = html;
  // FIX1: Notify collab peers of state change
