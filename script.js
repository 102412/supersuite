...
<nav class="ss-block ss-nav" style="background:${navBg};color:${data.textColor};position:${data.sticky?'sticky':'relative'};top:0;z-index:100;border-bottom:1px solid rgba(0,0,0,0.08);padding:0 var(--section-pad);">
  ...
  <div class="ss-mobile-nav" id="mob-${navId}" style="display:none;position:fixed;inset:0;background:${navBg === 'transparent' ? 'rgba(0,0,0,0.97)' : navBg};z-index:9999;flex-direction:column;align-items:center;justify-content:center;gap:32px;">
    ...
    ${data.links.map(l => `<a href="#" onclick="document.getElementById('mob-${navId}').classList.remove('open')" style="color:${data.textColor};text-decoration:none;font-size:24px;font-weight:700;opacity:0.9;">${l}</a>`).join('')}
...
<\/script>` : ''}
  ...
  async editWithAI(sectionId, instruction) {
    ...
    const cssSnippet = (State.siteCSS || '').slice(0, 1500);

    try {
      const result = await callAIWorker([
        {
          role: 'system',
          content: `You are editing one HTML section of a website. Other sections: ${context}.
The site uses this CSS (already loaded globally — do NOT add <style> tags):
${cssSnippet}
Modify the section exactly per the instruction. Keep the same general structure unless told to change it.
Return ONLY the complete modified HTML for this section. No explanation, no markdown fences.`
        },
  ...
  async addSection(description) {
    ...
    const existingSnippet = State.sections.map(s => s.html).join('\n').slice(0, 2500);
    const cssSnippet = (State.siteCSS || '').slice(0, 1500);
    ...
    try {
      const result = await callAIWorker([
        {
          role: 'system',
          content: `You are adding a new section to an existing website. Match the site's design system exactly — same fonts, colors, spacing, aesthetic.
Site CSS (use these variables and patterns):
${cssSnippet}

Existing sections for style reference:
${existingSnippet}

Generate ONLY the new HTML section. No <style> tags, no explanation, no markdown.`
        },
        { role: 'user', content: `Add this section: ${description}` }
...
async function generateCustomSiteHTML(info) {
  ...
  const prompt = `${biz} — ${ind}. ${svcs ? 'Services: ' + svcs + '.' : ''} ${loc ? 'Based in ' + loc + '.' : ''}`;

  // Step 1: Extract structured info
  const infoRaw = await callAIWorker([
    { role:'system', content:'Extract business info. Respond ONLY with valid JSON, no markdown:\n{"businessName":"","businessType":"","industry":"","location":"","targetCustomers":"","primaryGoal":"get calls|get bookings|sell products|show portfolio|build trust","tone":"professional|friendly|bold|luxury|playful"}' },
    { role:'user',   content: prompt }
  ], { max_tokens:300, temperature:0.3 });
  const bizInfo = _ssjson(infoRaw, { businessName:biz, businessType:ind, industry:ind, location:loc, targetCustomers:'customers', primaryGoal:'get calls', tone });

  // Step 2: Write copy
  const copyRaw = await callAIWorker([
    { role:'system', content:'You are a world-class copywriter. Write unique website copy. Respond ONLY with valid JSON, no markdown:\n{"heroHeadline":"","heroSub":"","heroCTA":"","f1Title":"","f1Body":"","f2Title":"","f2Body":"","f3Title":"","f3Body":"","t1Quote":"","t1Name":"","t2Quote":"","t2Name":"","ctaHeading":"","ctaBody":"","ctaCTA":"","footerTagline":""}' },
    { role:'user',   content:'Business: ' + JSON.stringify(bizInfo) }
...
const Onboarding = {
  ...
  reset() {
    ...
    this._step = 0;
    this._data = { businessName:'', industry:'', vibes:[], customers:'', palette:'', goal:'' };
    this._designs = [];
...
Make each design VISUALLY DISTINCT. Use different fonts, colors, border-radius, and layouts. The 3 designs should look nothing alike.`;
  ...
  /* ── STEP RENDERERS ─────────────────────────────────────────── */
  _steps: ['basics','vibes','customers','palette','logo','goal','generating','pick'],
  ...
  _renderProgress() {
    ...
    if (!el) return;
    const steps = 6;
    const pct = Math.min(100, (this._step / steps) * 100);
    el.innerHTML = `<div class="ob-progress-bar" style="width:${pct}%"></div>`;
  },
  ...
  _renderStep() {
    ...
    const d = this._data;
    let html = '', canProceed = true;

    if (this._step === 0) {
      html = `
        <div class="ob-eyebrow">STEP 1 OF 6 · LET'S START</div>
        <h2 class="ob-title">What's your business?</h2>
        <p class="ob-sub">We'll use this to personalise your entire site.</p>
        <label class="ob-field">
          <span class="ob-label">BUSINESS NAME</span>
          <input type="text" id="ob-biz-name" class="ob-input" placeholder="e.g. Luigi's Pizzeria" value="${this._esc(d.businessName)}" maxlength="60" autofocus>
        </label>
        <label class="ob-field">
      ...
      html = `
        <div class="ob-eyebrow">STEP 2 OF 6 · BRAND VIBE</div>
        <h2 class="ob-title">Pick 3 words that describe your brand</h2>
        <p class="ob-sub">These shape your fonts, layout, and feel.</p>
        ...
        <div class="ob-chip-hint" id="ob-vibe-hint">${d.vibes.length}/3 selected</div>`;
    } else if (this._step === 2) {
      html = `
        <div class="ob-eyebrow">STEP 3 OF 6 · YOUR CUSTOMERS</div>
        <h2 class="ob-title">Who visits your site?</h2>
        <p class="ob-sub">This shapes the tone and messaging.</p>
        <div class="ob-radio-list">
          ${this.CUSTOMERS.map(c => `
            <label class="ob-radio ${d.customers===c?'selected':''}">
              <input type="radio" name="ob-customers" value="${c}" ${d.customers===c?'checked':''} onchange="Onboarding._data.customers=this.value; Onboarding._highlightRadio(this)">
              <span>${c}</span>
            </label>`).join('')}
        </div>`;
    } else if (this._step === 3) {
      html = `
        <div class="ob-eyebrow">STEP 4 OF 6 · COLOR DIRECTION</div>
        <h2 class="ob-title">Which palette feels most like you?</h2>
        <p class="ob-sub">AI will generate the exact colors — this is just the direction.</p>
        <div class="ob-palette-grid">
          ${this.PALETTES.map(p => `
            <button type="button" class="ob-palette-card ${d.palette===p.id?'selected':''}" onclick="Onboarding._selectPalette('${p.id}')">
              <div class="ob-swatches">
                ${p.preview.map(c => `<div class="ob-swatch" style="background:${c}"></div>`).join('')}
              </div>
      ...
      html = `
        <div class="ob-eyebrow">STEP 5 OF 6 · LOGO</div>
        <h2 class="ob-title">Got a logo? Drop it here.</h2>
        <p class="ob-sub">Optional — if you have one, we'll place it in the nav. We'll extract your brand colors from it too.</p>
        <div class="ob-logo-drop" id="ob-logo-drop" onclick="document.getElementById('ob-logo-file').click()">
          <div class="ob-logo-icon">📎</div>
          <div class="ob-logo-text">Click to upload your logo</div>
          <div class="ob-logo-sub">PNG, SVG, JPG — any size</div>
          <input type="file" id="ob-logo-file" accept="image/*" style="display:none" onchange="Onboarding._handleLogo(this)">
        </div>
        <button type="button" class="ob-skip-link" onclick="Onboarding.next()">Skip — I'll add one later</button>`;
    } else if (this._step === 5) {
      html = `
        <div class="ob-eyebrow">STEP 6 OF 6 · PRIMARY GOAL</div>
        <h2 class="ob-title">What's the #1 thing your site needs to do?</h2>
        <p class="ob-sub">This shapes which blocks go front and centre.</p>
        <div class="ob-radio-list">
          ${this.GOALS.map(g => `
    ...
    foot.innerHTML = `
      ...
      <div class="ob-foot-right">
        ${!isLogo ? `<button type="button" class="ob-btn ob-btn-primary" onclick="Onboarding.next()">${isLast ? '✨ Build my 3 designs →' : 'Next →'}</button>` : ''}
      </div>`;
  },
  ...
  next() {
    ...
    if (this._step === 0) {
      ...
      if (!name) { this._shake(); showToast('⚠️ Business name required', 'Please enter your business name.', 'warning'); return; }
      if (!industry) { this._shake(); showToast('⚠️ Industry required', 'Please pick your industry.', 'warning'); return; }
      this._data.businessName = name;
      this._data.industry = industry;
      State.businessName = name;
      State.industry = industry;
    } else if (this._step === 1) {
      if (this._data.vibes.length < 1) { showToast('⚠️ Pick at least one vibe', '', 'warning'); return; }
    } else if (this._step === 2) {
      if (!this._data.customers) { showToast('⚠️ Select your customer type', '', 'warning'); return; }
    } else if (this._step === 3) {
      if (!this._data.palette) { showToast('⚠️ Pick a palette direction', '', 'warning'); return; }
    } else if (this._step === 5) {
      if (!this._data.goal) { showToast('⚠️ Pick your primary goal', '', 'warning'); return; }
      // All steps done — generate
      this._step = 6;
      this._renderModal();
      this._generateDesigns();
      return;
    }
  ...
  Onboarding.finish = async function() {
    ...
    try {
      const html = await generateCustomSiteHTML({
        ...
        businessType: d.businessType || 'service business',
        services:     d.services     || '',
        location:     d.location     || '',
        phone:        d.phone        || '',
...
const EmailAuth = {
  ...
  async submit() {
    ...
    const remember = !!document.getElementById('ea-remember')?.checked;
    if (errEl) errEl.textContent = '';

    if (!email || !email.includes('@') || email.length < 5) {
      return this._fail('Please enter a valid email address.');
    }
    if (!pwd || pwd.length < 6) {
    ...
    if (this._mode === 'signup') {
      ...
      const accounts = this._read();
      if (accounts[email]) return this._fail('An account with this email already exists. Try signing in.');
      let tier = 'free', tierLabel = 'Free Trial';
      if (bindCode) {
        const r = (typeof Auth !== 'undefined') ? Auth.verify(bindCode) : { ok: false };
        if (!r.ok) return this._fail('Invalid plan code. Leave blank for Free, or check your code.');
        tier = r.tier; tierLabel = r.label;
      }
      const salt = this._newSalt();
      const hash = await this._hash(pwd, salt);
      const token = this._newToken();
      accounts[email] = {
        email, name: name || email.split('@')[0], tier, tierLabel,
        salt, hash, token, createdAt: Date.now(), lastSignIn: Date.now(),
      };
      this._write(accounts);
      this.saveSession(email, token, remember);
      if (typeof showToast === 'function') showToast('🎉 Account created', 'Welcome to Supersuite, ' + accounts[email].name + '!', 'success');
      return this._activate(accounts[email], false);
    }

    // signin
    const accounts = this._read();
    const acc = accounts[email];
    if (!acc) return this._fail('No account found for that email. Click "Create Account" to sign up.');
    const hash = await this._hash(pwd, acc.salt);
    if (hash !== acc.hash) return this._fail('Incorrect password. Try again or click "Forgot password?"');
    acc.token = this._newToken();
    acc.lastSignIn = Date.now();
    this._write(accounts);
    this.saveSession(email, acc.token, remember);
    return this._activate(acc, false);
  },
