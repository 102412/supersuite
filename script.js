...
(function SSV27Cleanup(){
  const REMOVED_IDS = [
    ...
    'ai-generate-btn',     // top-bar "Generate with AI"
  ...
  function scrub(){
    ...
    document.querySelectorAll('.top-nav button, .nav-right button').forEach(b => {
      ...
      if (b.id === 'ai-generate-btn' || b.id === 'regen-btn') b.remove();
    });
  }
...
Make each design VISUALLY DISTINCT. Use different fonts, colors, border-radius, and layouts. The 3 designs should look nothing alike.`;
  ...
  _applyDesign(design) {
    ...
    order.forEach(type => {
      ...
      // Personalize each block type with real business data + design colors
      if (type === 'nav') {
        data.logo      = name;
        data.bgColor   = bg;
        data.textColor = text;
        data.ctaBgColor = primary;
        data.ctaText   = copy.cta2;
      }
      if (type === 'hero') {
        data.heading    = copy.h;
        ...
        data.btn2Color  = 'transparent';
        data.layout     = design.heroLayout || 'centered';
      ...
      if (type === 'features') {
        ...
        data.subheading = 'We make ' + industry + ' simple, reliable, and affordable.';
        data.bgColor    = bg;
        data.textColor  = text;
        data.accentColor = primary;
        if (data.cards) {
          const featureDefaults = [
            { icon:'⚡', title:'Fast Service',    body:'Quick turnaround, results you can count on.' },
            { icon:'🏆', title:'Expert Quality',  body:'Years of experience in ' + industry + ' you can trust.' },
            { icon:'💰', title:'Fair Pricing',    body:'Transparent quotes with no hidden fees.' },
          ];
          data.cards = data.cards.map((c,i) => ({ ...c, ...(featureDefaults[i] || {}) }));
        }
      }
      if (type === 'testimonials') {
        data.heading    = 'What Our Clients Say';
        data.subheading = 'Real reviews from real ' + industry + ' customers.';
      ...
      if (type === 'pricing') {
        ...
        data.subheading = 'Simple, transparent ' + industry + ' rates.';
        data.bgColor    = bg;
        data.textColor  = text;
        data.accentColor = primary;
      }
      if (type === 'cta') {
        ...
        data.subheading = 'Contact ' + name + ' today — we will get back to you within the hour.';
        data.btnText    = copy.cta;
        data.btn2Text   = copy.cta2;
        data.bgColor    = primary;
        data.textColor  = '#ffffff';
