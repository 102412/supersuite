      ...
      return `<div class="ss-section-wrapper" data-section-id="${s.id}"
        ...
        <div class="ss-sec-ctrl" style="all:initial;display:none;position:absolute;top:10px;right:10px;z-index:2147483647;flex-direction:row;gap:6px;align-items:center;pointer-events:all;">
          <button onclick="window.parent.postMessage({type:'openSectionEditor',id:'${s.id}'},'*')"
            style="all:initial;display:inline-block;background:#ff6b35;color:#fff;border:none;border-radius:7px;padding:7px 13px;font-size:12px;font-weight:700;cursor:pointer;font-family:sans-serif;white-space:nowrap;line-height:1;box-shadow:0 2px 8px rgba(0,0,0,.4);">✏ Edit</button>
  ...
  const previewInteractScript = !forExport ? `
    <script>
      ...
      (function(){
        ...
        // Elements that are directly editable (have text, not wrappers)
        var TEXT_TAGS = ['H1','H2','H3','H4','H5','H6','P','BUTTON','A','LABEL','LI','CAPTION','FIGCAPTION','SPAN','TD','TH'];
        // Single-line elements: Enter saves instead of inserting newline
        var SINGLE_LINE_TAGS = ['H1','H2','H3','H4','H5','H6','BUTTON','A','LABEL','CAPTION'];
        // Never editable
        var SKIP_TAGS = ['IMG','VIDEO','AUDIO','INPUT','TEXTAREA','SELECT','SVG','CANVAS','IFRAME'];

        function isSkip(el) {
          return !el || SKIP_TAGS.indexOf(el.tagName) !== -1;
        }

        function isWrapper(el) {
          // Skip large layout containers — only allow specific text-bearing tags
        ...
        function findEditable(target) {
          ...
          while (el && el !== document.body && depth < 8) {
            if (!isSkip(el)) {
              // Prefer elements that directly contain text nodes
              if (TEXT_TAGS.indexOf(el.tagName) !== -1 && hasDirectText(el)) return el;
              // Also accept if it's a known text tag with any text at all
              if (TEXT_TAGS.indexOf(el.tagName) !== -1 && el.textContent.trim().length > 0 && el.textContent.trim().length < 500) return el;
            }
            el = el.parentElement;
            depth++;
          }
        ...
        function isSingleLine(el) {
          return SINGLE_LINE_TAGS.indexOf(el.tagName) !== -1;
        }
        ...
        function activate(el) {
          ...
          // Store both textContent (for matching) and innerHTML (for cancel restore)
          el.dataset.ssOrigText = el.textContent;
          el.dataset.ssOrigHtml = el.innerHTML;

          el.contentEditable = 'true';

          // Visual state: orange outline + subtle glow background
          el.style.setProperty('outline',          '1px solid #ff6b35',       'important');
          el.style.setProperty('outline-offset',   '2px',                     'important');
          el.style.setProperty('border-radius',    '2px',                     'important');
          // Only add bg glow for elements that likely don't have their own bg
          if (!el.style.backgroundColor || el.style.backgroundColor === '' || el.style.backgroundColor === 'transparent') {
            el.dataset.ssAddedBg = '1';
            el.style.setProperty('background-color', 'rgba(255,107,53,0.06)', 'important');
          }
        ...
        function deactivate(el, cancel) {
          ...
          if (cancel) {
            // Restore original HTML (preserves nested tags like <strong>)
        ...
        // Double-click to activate (avoids conflict with single-click block selection)
        ...
        // Mousedown outside active element → save and deactivate
        document.addEventListener('mousedown', function(e) {
          if (!active) return;
          if (e.target === active || active.contains(e.target)) return;
          // Don't deactivate on clicking the block controls
          if (e.target.closest('.ss-block-controls,.ss-sec-ctrl')) return;
        ...
        document.addEventListener('keydown', function(e) {
          ...
          if (e.key === 'Escape') {
            e.preventDefault();
            e.stopPropagation();
            deactivate(active, true); // cancel — restore original
          }
          if (e.key === 'Enter') {
            ...
            // Multi-line (p, span, etc): allow natural Enter → newline via Shift+Enter; bare Enter also saves
            else if (!e.shiftKey) {
              e.preventDefault();
              deactivate(active, false);
            }
            // Shift+Enter on multi-line → inserts newline (default browser behavior)
          }
        }, true);

        // Blur fallback (Tab key, focus moved programmatically)
        document.addEventListener('blur', function(e) {
          if (!active || e.target !== active) return;
          // Small delay so mousedown handler fires first
          setTimeout(function() {
      ...
      (function(){
        ...
        document.addEventListener('click', function(e) {
          ...
          // Mode A: text element — handled by existing dblclick handler
          // Mode B: background area
          if (!isTextEl(t)) {
...
function applyInlineEdit(msg) {
  ...
  if (sectionId && State.sections.length) {
    const sec = State.sections.find(s => s.id === sectionId);
    if (!sec) return;
    // Replace ONLY inside text nodes (between > and <), not in attribute values
...
Make each design VISUALLY DISTINCT. Use different fonts, colors, border-radius, and layouts. The 3 designs should look nothing alike.`;
  ...
  /* ── BRAND INTERVIEW — 3 steps, only name required ──────────── */
  _steps: ['basics', 'brand', 'details'],
  ...
  _renderStep() {
    ...
    // ── Step 0: The basics (only required step) ──────────────────
    if (this._step === 0) {
      body.innerHTML = `
  ...
  next() {
    ...
    if (this._step === 0) {
      ...
      if (!name) { this._shake(); showToast('⚠️ Just need your business name', '', 'warning'); return; }
      d.businessName = name;
      ...
      // All optional — just collect whatever's there
      d.tone           = document.getElementById('ob-tone-select')?.value || d.tone || '';
      d.differentiator = g('ob-differentiator');
      d.targetCustomer = g('ob-customer');
      // vibes already updated via _toggleVibe

    } else if (this._step === 2) {
      // Last step — disable button immediately so user knows it worked
