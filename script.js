...
/* ═══════════════════════════════════════════════════════════════════════
   SUPERSUITE V27 — PHASE 1 CLEANUP MODULE
   ─────────────────────────────────────────────────────────────────────
   Removes / neutralizes legacy features that will not ship in v27:
     1. SS Studio (Studio Mode dual-pane) — now a separate product
     2. Floating AI chat pill (#ai-chat-fab) — replaced in Phase 2
     3. Dashboard (📊 Dashboard panel) — removed entirely
     4. AI Regenerate Site button + modal — removed
     5. AI Generate-with-AI top-bar button — removed
...
(function SSV27Cleanup(){
  const REMOVED_IDS = [
    ...
    // 'ai-generate-btn' — kept active (Generate with AI button)
    'regen-btn',           // top-bar "Regenerate"
    'regen-modal',         // regenerate modal
    // 'studio-launch-btn' — kept (Studio Agent restored)
    // 'studio-mode-root'  — kept (Studio Agent restored)
    'ss-dashboard-btn',    // top-bar "📊 Dashboard"
  ...
  function scrub(){
    ...
    document.querySelectorAll('.top-nav button, .nav-right button').forEach(b => {
      ...
      if (txt.startsWith('📊 Dashboard')) b.remove();
  ...
  function neutralize(){
    const STUBS = {
      ...
      // StudioMode methods restored — Studio Agent active
    };
...
(function SSV27KillSessions(){
  ...
  try {
    // Clear sessionStorage entirely (safe — only Supersuite uses it)
...
const State = {
  ...
  blockIdCounter: 1,
  // ── SSections (dynamic AI mode) ──────────────────────────────
  sections:          [],  // [{id, label, html, visible}] — AI-generated per user
...
<nav class="ss-block ss-nav" style="background:${navBg};color:${data.textColor};position:${data.sticky?'sticky':'relative'};top:0;z-index:100;border-bottom:1px solid rgba(0,0,0,0.08);padding:0 var(--section-pad);">
  ...
  <div class="ss-mobile-nav" id="mob-${navId}" style="display:none;position:fixed;inset:0;background:${navBg === 'transparent' ? 'rgba(0,0,0,0.97)' : navBg};z-index:9999;flex-direction:column;align-items:center;justify-content:center;gap:32px;">
    <button class="ss-mobile-nav-close" onclick="document.getElementById('mob-${navId}').classList.remove('open')" style="position:absolute;top:24px;right:24px;background:none;border:none;color:${data.textColor};font-size:32px;cursor:pointer;line-height:1;">×</button>
    ...
    <a href="${data.ctaLink}" style="background:${data.ctaBgColor};color:white;padding:14px 36px;border-radius:var(--btn-radius);font-size:16px;font-weight:700;text-decoration:none;margin-top:8px;">${data.ctaText} →</a>
  </div>
</nav>
<style>
...
<footer class="ss-block ss-footer" style="background:${data.bgColor};color:${data.textColor};padding:64px var(--section-pad) 32px;">
  ...
  <div style="width:100%;height:${data.height||'480px'};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:#1a1a2e;color:rgba(255,255,255,0.25);font-family:'DM Sans',sans-serif;">
    ...
    <span style="font-size:14px;">No photo selected — click Edit to browse Unsplash</span>
  </div>
</section>`;
    }
...
────────────────────────────────────────────────────────────────── */
/* ══════════════════════════════════════════════════════════════════
   SSECTIONS — 100% Dynamic AI Section System (SSV28+)
   ─────────────────────────────────────────────────────────────────
   Every user gets a completely unique site generated from scratch.
   No shared block templates. AI writes raw HTML → parsed into
...
const SSections = {
  ...
  /* ── Parse a full AI-generated HTML string into State.sections ── */
  load(fullHTML, prompt) {
    ...
    // Clear old blocks — we're in sections mode now
    State.blocks = [];
    ...
    updateLayers();
    switchTab('layers');
    showToast('✨ Site generated', State.sections.length + ' unique sections built for you', 'success');
    return true;
  },

  /* ── Full preview HTML from sections ─────────────────────── */
  buildPreview(forExport) {
    ...
    const sectionsHTML = visible.map(s => {
      ...
      return `<div class="ss-section-wrapper" data-section-id="${s.id}"
        ...
        onmouseenter="this.style.outlineColor='rgba(255,107,53,0.5)';this.querySelector('.ss-sec-ctrl').style.display='flex';"
        onmouseleave="this.style.outlineColor='transparent';this.querySelector('.ss-sec-ctrl').style.display='none';">
        ${s.html}
        <div class="ss-sec-ctrl" style="position:absolute;top:10px;right:10px;display:none;z-index:500;gap:6px;align-items:center;">
          <button onclick="window.parent.postMessage({type:'openSectionEditor',id:'${s.id}'},'*')"
            style="background:#ff6b35;color:#fff;border:none;border-radius:7px;padding:7px 13px;font-size:12px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap;">✏ Edit</button>
          <button onclick="window.parent.postMessage({type:'moveSection',id:'${s.id}',dir:'up'},'*')"
            style="background:#1a1a2e;color:#fff;border:none;border-radius:7px;padding:7px 10px;font-size:12px;cursor:pointer;">↑</button>
          ...
          <button onclick="window.parent.postMessage({type:'deleteSection',id:'${s.id}'},'*')"
            style="background:#ef4444;color:#fff;border:none;border-radius:7px;padding:7px 10px;font-size:12px;cursor:pointer;">🗑</button>
        </div>
      </div>`;
    }).join('\n');
...
${!forExport ? `<script>
document.querySelectorAll('.ss-section-wrapper').forEach(function(w){
  var ctrl=w.querySelector('.ss-sec-ctrl');
  w.addEventListener('mouseenter',function(){if(ctrl)ctrl.style.display='flex';w.style.outlineColor='rgba(255,107,53,0.5)';});
  w.addEventListener('mouseleave',function(){if(ctrl)ctrl.style.display='none';w.style.outlineColor='transparent';});
});
(function(){
  document.addEventListener('click',function(e){
    var el=e.target;while(el&&el.tagName!=='A')el=el.parentElement;
    if(el&&el.tagName==='A'){var h=el.getAttribute('href')||'';if(h.charAt(0)==='#'||!h||h.indexOf('javascript')===0)return;e.preventDefault();}
  },true);
})();
<\/script>` : ''}
</body>
...
1. Use ONLY inline styles — never add <style> tags or new CSS classes
...
Modify the section exactly per the instruction. Return ONLY the modified HTML. No explanation, no markdown.`
      ...
      if (clean && clean.length > 30) {
        ...
        refreshPreview();
        this.updatePanel();
        showToast('✅ Section updated', 'AI applied your changes', 'success');
      } else {
        showToast('⚠️ No change', 'AI returned empty. Try rephrasing.', 'error');
      }
    } catch(e) {
      showToast('⚠️ Edit failed', 'Could not reach AI. Check connection.', 'error');
    }
    openSectionEditor(sectionId);
  },

  /* ── AI generate a brand-new section ─────────────────────── */
  async addSection(description) {
    if (!description.trim()) return;

    const cssVars = _extractCSSVars(State.siteCSS);
    // Use the features or hero section as the style reference — richest example
    ...
    showToast('⏳ Generating…', 'AI is building your new section', 'info');
...
1. Use ONLY inline styles on every element — never write <style> tags or CSS classes
2. These CSS variables are already defined globally — use them in every color/font/radius decision:
   ${cssVars}
...
Return ONLY the complete HTML section. No <style> tags. No explanation. No markdown fences.`
      ...
      this.updatePanel();
      updateLayers();
      showToast('✅ Section added', label + ' added to your site', 'success');

    } catch(e) {
      showToast('⚠️ Failed', 'Could not generate section. Try again.', 'error');
    }
  },
  ...
  delete(id) {
    ...
    updateLayers();
    closeRightPanel();
    showToast('🗑 Removed', label + ' deleted', 'info');
  },
  ...
  /* ── Re-render the sections list in the sidebar ──────────── */
  updatePanel() {
    ...
    if (!State.sections.length) {
      container.innerHTML = `
        <div style="text-align:center;padding:32px 16px;color:var(--ui-text2);">
          ...
          <p style="font-size:13px;line-height:1.6;">Complete onboarding — AI will generate<br/>a 100% unique site for your business.</p>
        </div>`;
      return;
    }

    const ICONS = {
      'Navigation':'🧭','Hero':'⚡','Features':'✨','Testimonials':'💬',
      'Pricing':'💎','Call to Action':'🎯','Gallery':'🖼️','About':'👤',
      'Footer':'📌','Contact':'📋','FAQ':'❓','New Section':'➕','Header':'🏠'
    };
    ...
    container.innerHTML = State.sections.map((s,i) => `
      <div class="layer-item ${State.selectedSectionId === s.id ? 'active' : ''}"
        ...
        <span class="layer-icon">${ICONS[s.label] || '📄'}</span>
        <span class="layer-label" style="flex:1;">${s.label}</span>
        <div class="layer-actions">
          <button class="layer-btn" onclick="event.stopPropagation();openSectionEditor('${s.id}')" title="Edit">✏️</button>
          <button class="layer-btn" onclick="event.stopPropagation();SSections.move('${s.id}','up')" title="Move up">↑</button>
          <button class="layer-btn" onclick="event.stopPropagation();SSections.move('${s.id}','down')" title="Move down">↓</button>
          <button class="layer-btn danger" onclick="event.stopPropagation();SSections.delete('${s.id}')" title="Delete">🗑</button>
        </div>
      </div>`).join('');
  },
};
...
/* ══════════════════════════════════════════════════════════════════
   REGEN SYSTEM — Tier-limited site regeneration
══════════════════════════════════════════════════════════════════ */
const RegenSystem = (function(){
  ...
  function openModal() {
    ...
    if (line) {
      line.innerHTML = rem > 0
        ...
        : `<span style="color:#ef4444;font-weight:700;">No regenerations left</span> on your ${_tier()} plan. <a href="https://supersuite-checkout.vercel.app" target="_blank" style="color:#ff6b35;">Upgrade →</a>`;
    }
  ...
  async function run(mode) {
    ...
    showToast('🔄 Regenerating…', mode === 'design' ? 'Rebuilding design, keeping your content' : 'Building fresh from scratch', 'info');
    ...
    try {
      ...
      if (html && SSections.load(html, fullPrompt)) {
        _bump();
        showToast('✅ Regenerated!', _remaining() + ' regen' + (_remaining()!==1?'s':'') + ' remaining', 'success');
      }
    } catch(e) {
      showToast('⚠️ Regen failed', 'Could not reach AI. Try again.', 'error');
    }
  }
...
/* ── Open right panel for a section ─────────────────────────── */
function openSectionEditor(id) {
  ...
  if (rpTitle) rpTitle.textContent = '✏️ ' + sec.label;
  rpContent.innerHTML = `
    <div style="padding:4px 0;">

      <div style="margin-bottom:16px;">
        <label style="font-size:11px;font-weight:700;color:var(--ui-accent);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:8px;">✨ Edit with AI</label>
        <textarea id="sec-ai-input" rows="3"
          placeholder="e.g. make the background darker, change heading to mention free estimates, add a phone number, make buttons more rounded…"
          style="width:100%;background:var(--ui-bg2);border:1px solid var(--ui-border);border-radius:8px;color:var(--ui-text);padding:10px;font-size:13px;resize:vertical;font-family:'DM Sans',sans-serif;outline:none;line-height:1.5;transition:border-color 0.2s;"
        ...
        <button onclick="SectionEditorApply()" id="sec-ai-btn"
          style="width:100%;margin-top:8px;padding:11px;background:#ff6b35;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:14px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:background 0.2s;"
          onmouseover="this.style.background='#e85520'" onmouseout="this.style.background='#ff6b35'">Apply Changes →</button>
        <p style="font-size:11px;color:var(--ui-text2);margin-top:6px;text-align:center;">⌘↵ to apply · AI edits only this section</p>
      </div>

      <hr style="border:none;border-top:1px solid var(--ui-border);margin:14px 0;"/>

      <div style="margin-bottom:16px;">
        <label style="font-size:11px;font-weight:700;color:var(--ui-accent);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:8px;">➕ Add a New Section</label>
        <input id="sec-add-input" type="text"
          placeholder="e.g. FAQ section, team grid, contact form…"
          style="width:100%;background:var(--ui-bg2);border:1px solid var(--ui-border);border-radius:8px;color:var(--ui-text);padding:9px 12px;font-size:13px;font-family:'DM Sans',sans-serif;outline:none;transition:border-color 0.2s;"
          onfocus="this.style.borderColor='rgba(255,107,53,0.5)'" onblur="this.style.borderColor='var(--ui-border)'"
          onkeydown="if(event.key==='Enter')SectionEditorAdd()"/>
        <button onclick="SectionEditorAdd()"
          style="width:100%;margin-top:8px;padding:10px;background:rgba(255,107,53,0.1);color:#ff6b35;border:1px solid rgba(255,107,53,0.3);border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.2s;"
          onmouseover="this.style.background='rgba(255,107,53,0.2)'" onmouseout="this.style.background='rgba(255,107,53,0.1)'">＋ Generate &amp; Match Style</button>
      </div>

      <hr style="border:none;border-top:1px solid var(--ui-border);margin:14px 0;"/>

      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button onclick="SSections.move('${id}','up')"   style="flex:1;min-width:60px;padding:8px;background:var(--ui-bg2);border:1px solid var(--ui-border);color:var(--ui-text);border-radius:7px;font-size:12px;cursor:pointer;">↑ Up</button>
        <button onclick="SSections.move('${id}','down')" style="flex:1;min-width:60px;padding:8px;background:var(--ui-bg2);border:1px solid var(--ui-border);color:var(--ui-text);border-radius:7px;font-size:12px;cursor:pointer;">↓ Down</button>
        <button onclick="SSections.delete('${id}')"       style="padding:8px 12px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);color:#ef4444;border-radius:7px;font-size:12px;cursor:pointer;">🗑 Delete</button>
      </div>
    </div>`;
...
function SectionEditorApply() {
  ...
  SSections.editWithAI(State.selectedSectionId, val)
    .finally(() => { if (btn) { btn.textContent = 'Apply Changes →'; btn.disabled = false; } });
}
...
/* ── Generate a full custom site for the builder (onboarding) ── */
async function generateCustomSiteHTML(info) {
  ...
  const vibs = info.vibes          || '';
  const cta  = info.cta            || '';
  const desc = info.description    || '';
  const extr = info.extra          || '';

  // Rich brand context for AI — the more detail, the more unique the output
  const brandContext = [
    `Business: ${biz}`,
    `Industry: ${ind}`,
    desc        ? `What they do: ${desc}` : '',
    loc         ? `Location: ${loc}` : '',
    svcs        ? `Services: ${svcs}` : '',
    diff        ? `What makes them different: ${diff}` : '',
    cust        ? `Ideal customer: ${cust}` : '',
    vibs        ? `Brand personality: ${vibs}` : '',
    tone        ? `Tone: ${tone}` : '',
    cta         ? `CTA: ${cta}` : '',
    info.phone  ? `Phone: ${info.phone}` : '',
    extr        ? `Extra context: ${extr}` : '',
  ].filter(Boolean).join('\n');

  // Step 1: Extract / enrich structured info
  const infoRaw = await callAIWorker([
    { role:'system', content:'You are a brand strategist. Extract and infer structured info from this brand brief. Respond ONLY with valid JSON, no markdown:\n{"businessName":"","businessType":"","industry":"","location":"","targetCustomers":"","primaryGoal":"get calls|get bookings|sell products|show portfolio|build trust","tone":"professional|friendly|bold|luxury|playful","uniqueValue":"one sentence differentiator"}' },
    { role:'user',   content: brandContext }
  ], { max_tokens:400, temperature:0.3 });
  const bizInfo = _ssjson(infoRaw, { businessName:biz, businessType:ind, industry:ind, location:loc, targetCustomers:cust||'customers', primaryGoal:'get calls', tone, uniqueValue:diff });

  // Step 2: Write copy (skip if design-only regen — reuse existing)
  let copyRaw;
  if (info.regenMode === 'design' && info.existingCopy) {
    // Extract copy fields from existing text instead of generating new
    copyRaw = await callAIWorker([
      { role:'system', content:'Extract website copy fields from this existing site text. Respond ONLY with valid JSON, no markdown:\n{"heroHeadline":"","heroSub":"","heroCTA":"","f1Title":"","f1Body":"","f2Title":"","f2Body":"","f3Title":"","f3Body":"","t1Quote":"","t1Name":"","t2Quote":"","t2Name":"","ctaHeading":"","ctaBody":"","ctaCTA":"","footerTagline":""}' },
      { role:'user',   content: info.existingCopy }
    ], { max_tokens:700, temperature:0.2 });
  } else {
    copyRaw = await callAIWorker([
      { role:'system', content:'You are a world-class direct-response copywriter. Write specific, punchy, conversion-focused website copy for this exact business. Every word must be tailored — no generic phrases like "we go above and beyond" or "industry-leading". Use the differentiator and tone. Respond ONLY with valid JSON, no markdown:\n{"heroHeadline":"","heroSub":"","heroCTA":"","f1Title":"","f1Body":"","f2Title":"","f2Body":"","f3Title":"","f3Body":"","t1Quote":"","t1Name":"","t2Quote":"","t2Name":"","ctaHeading":"","ctaBody":"","ctaCTA":"","footerTagline":""}' },
      { role:'user',   content: brandContext + '\n\nStructured info: ' + JSON.stringify(bizInfo) }
    ], { max_tokens:900, temperature:0.75 });
  }
  const copy = _ssjson(copyRaw, { heroHeadline: biz, heroSub:'Professional services you can trust.', heroCTA:'Get in Touch', ctaHeading:'Ready to get started?', ctaBody:'Contact us today.', ctaCTA:'Call Now', footerTagline: biz + ' — ' + ind });

  // Step 3: Design spec
  const designRaw = await callAIWorker([
    { role:'system', content:'You are a UI designer. Return ONLY valid JSON, no markdown:\n{"primaryColor":"#hex","secondaryColor":"#hex","accentColor":"#hex","bgColor":"#hex","textColor":"#hex","headingFont":"Google Font name","bodyFont":"Google Font name","borderRadius":"8px","heroLayout":"centered|split|bold"}' },
    { role:'user',   content:'Tone: ' + bizInfo.tone + '. Industry: ' + bizInfo.industry }
  ], { max_tokens:250, temperature:0.8 });
  const design = _ssjson(designRaw, { primaryColor:'#ff6b35', secondaryColor:'#1a1a2e', accentColor:'#ffd166', bgColor:'#ffffff', textColor:'#1a1a2e', headingFont:'Syne', bodyFont:'DM Sans', borderRadius:'10px', heroLayout:'centered' });

  // Step 4: Build full HTML (custom per user)
  const p = design.primaryColor   || '#ff6b35';
  const s = design.secondaryColor || '#1a1a2e';
  const a = design.accentColor    || '#ffd166';
  const bg = design.bgColor       || '#ffffff';
  const tc = design.textColor     || '#1a1a2e';
  const hf = design.headingFont   || 'Syne';
  const bf = design.bodyFont      || 'DM Sans';
  const br = design.borderRadius  || '10px';
  const hl = design.heroLayout    || 'centered';

  const heroBg = hl === 'bold' ? `linear-gradient(135deg,${p},${s})` : bg;
  const heroTc = hl === 'bold' ? '#fff' : tc;
...
<link href="https://fonts.googleapis.com/css2?family=${encodeURIComponent(hf)}:wght@700;800;900&family=${encodeURIComponent(bf)}:wght@400;500;600&display=swap" rel="stylesheet">
<style>
*{box-sizing:border-box;margin:0;padding:0}
:root{--p:${p};--s:${s};--a:${a};--bg:${bg};--tc:${tc};--br:${br};--fh:'${hf}',sans-serif;--fb:'${bf}',sans-serif;}
body{background:var(--bg);color:var(--tc);font-family:var(--fb);line-height:1.65}
a{color:inherit;text-decoration:none}
.container{max-width:1160px;margin:0 auto;padding:0 28px}
.btn{display:inline-block;padding:14px 30px;border-radius:var(--br);font-weight:700;font-size:15px;cursor:pointer;transition:.2s;border:none;}
.btn-p{background:var(--p);color:#fff;}
.btn-p:hover{opacity:.88;transform:translateY(-2px)}
.btn-o{background:transparent;color:${heroTc};border:2px solid ${heroTc==='#fff'?'rgba(255,255,255,0.5)':p};}
.btn-o:hover{background:${heroTc==='#fff'?'rgba(255,255,255,0.12)':p};color:#fff}
@media(max-width:768px){.hide-mob{display:none!important}.stack{grid-template-columns:1fr!important}}
</style>
</head>
<body>

<nav style="background:${bg};border-bottom:1px solid rgba(0,0,0,0.07);padding:0;position:sticky;top:0;z-index:100;box-shadow:0 2px 16px rgba(0,0,0,0.06);">
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;height:68px;">
    <div style="font-family:var(--fh);font-weight:900;font-size:22px;color:var(--p);letter-spacing:-0.5px;">${_esc(biz)}</div>
    <div class="hide-mob" style="display:flex;gap:28px;font-size:14px;font-weight:500;opacity:.75;">
      <a href="#features" style="transition:opacity .2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.75">Services</a>
      <a href="#testimonials" style="transition:opacity .2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.75">Reviews</a>
      <a href="#cta" style="transition:opacity .2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.75">Contact</a>
    </div>
    <a href="#cta" class="btn btn-p" style="font-size:14px;padding:10px 22px;">${_esc(copy.heroCTA)}</a>
  </div>
</nav>

<section class="ss-hero" style="background:${heroBg};color:${heroTc};padding:100px 0 80px;text-align:${hl==='split'?'left':'center'};">
  <div class="container">
    <div style="${hl==='split'?'display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center':'max-width:760px;margin:0 auto'}">
      <div>
        <h1 style="font-family:var(--fh);font-size:clamp(36px,6vw,68px);font-weight:900;line-height:1.05;letter-spacing:-0.03em;margin-bottom:22px;${hl!=='bold'?'color:var(--p)':'color:#fff'}">${_esc(copy.heroHeadline)}</h1>
        <p style="font-size:18px;opacity:.78;margin-bottom:36px;line-height:1.7;">${_esc(copy.heroSub)}</p>
        <div style="display:flex;gap:14px;${hl!=='split'?'justify-content:center':''}flex-wrap:wrap;">
          <a href="#cta" class="btn btn-p">${_esc(copy.heroCTA)}</a>
          <a href="#features" class="btn btn-o">Learn More</a>
        </div>
      </div>
      ${hl==='split'?`<div style="border-radius:calc(var(--br)*2);overflow:hidden;aspect-ratio:4/3;background:${p}22;display:flex;align-items:center;justify-content:center;font-size:64px;">🏢</div>`:''}
    </div>
  </div>
</section>

<section class="ss-features" id="features" style="background:#f8f9fc;padding:90px 0;">
  <div class="container">
    <div style="text-align:center;margin-bottom:56px;">
      <div style="font-size:12px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--p);margin-bottom:12px;">Why Choose Us</div>
      <h2 style="font-family:var(--fh);font-size:clamp(26px,4vw,44px);font-weight:800;letter-spacing:-0.02em;">${_esc(copy.f1Title ? 'What Makes Us Different' : 'Our Services')}</h2>
    </div>
    <div class="stack" style="display:grid;grid-template-columns:repeat(3,1fr);gap:28px;">
      <div style="background:#fff;border-radius:var(--br);padding:32px;box-shadow:0 4px 20px rgba(0,0,0,.07);border:1px solid rgba(0,0,0,.05);transition:.2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="font-size:36px;margin-bottom:16px;">⚡</div>
        <h3 style="font-family:var(--fh);font-size:19px;font-weight:700;margin-bottom:10px;">${_esc(copy.f1Title||'Fast & Reliable')}</h3>
        <p style="font-size:14px;opacity:.65;line-height:1.7;">${_esc(copy.f1Body||'Quick turnaround with results you can count on.')}</p>
      </div>
      <div style="background:#fff;border-radius:var(--br);padding:32px;box-shadow:0 4px 20px rgba(0,0,0,.07);border:1px solid rgba(0,0,0,.05);transition:.2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="font-size:36px;margin-bottom:16px;">🏆</div>
        <h3 style="font-family:var(--fh);font-size:19px;font-weight:700;margin-bottom:10px;">${_esc(copy.f2Title||'Expert Quality')}</h3>
        <p style="font-size:14px;opacity:.65;line-height:1.7;">${_esc(copy.f2Body||'Years of industry experience delivering excellence.')}</p>
      </div>
      <div style="background:#fff;border-radius:var(--br);padding:32px;box-shadow:0 4px 20px rgba(0,0,0,.07);border:1px solid rgba(0,0,0,.05);transition:.2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="font-size:36px;margin-bottom:16px;">💰</div>
        <h3 style="font-family:var(--fh);font-size:19px;font-weight:700;margin-bottom:10px;">${_esc(copy.f3Title||'Fair Pricing')}</h3>
        <p style="font-size:14px;opacity:.65;line-height:1.7;">${_esc(copy.f3Body||'Transparent pricing with no hidden fees.')}</p>
      </div>
    </div>
  </div>
</section>

<section class="ss-testimonials" id="testimonials" style="background:${bg};padding:90px 0;text-align:center;">
  <div class="container">
    <div style="font-size:12px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:var(--p);margin-bottom:12px;">Reviews</div>
    <h2 style="font-family:var(--fh);font-size:clamp(26px,4vw,44px);font-weight:800;letter-spacing:-0.02em;margin-bottom:52px;">What Our Clients Say</h2>
    <div class="stack" style="display:grid;grid-template-columns:repeat(2,1fr);gap:24px;text-align:left;">
      <div style="background:#fff;border-radius:var(--br);padding:28px;border:1px solid rgba(0,0,0,.07);box-shadow:0 4px 16px rgba(0,0,0,.06);">
        <div style="color:#fbbf24;font-size:15px;margin-bottom:12px;">★★★★★</div>
        <p style="font-size:15px;line-height:1.7;opacity:.8;font-style:italic;margin-bottom:18px;">"${_esc(copy.t1Quote||'Outstanding service — exceeded all our expectations.')}"</p>
        <div style="font-weight:700;font-size:14px;color:var(--p);">${_esc(copy.t1Name||'Happy Customer')}</div>
      </div>
      <div style="background:#fff;border-radius:var(--br);padding:28px;border:1px solid rgba(0,0,0,.07);box-shadow:0 4px 16px rgba(0,0,0,.06);">
        <div style="color:#fbbf24;font-size:15px;margin-bottom:12px;">★★★★★</div>
        <p style="font-size:15px;line-height:1.7;opacity:.8;font-style:italic;margin-bottom:18px;">"${_esc(copy.t2Quote||'Professional, fast, and incredibly reliable. Highly recommend.')}"</p>
        <div style="font-weight:700;font-size:14px;color:var(--p);">${_esc(copy.t2Name||'Satisfied Client')}</div>
      </div>
    </div>
  </div>
</section>

<section class="ss-cta" id="cta" style="background:linear-gradient(135deg,${p},${s});color:#fff;padding:100px 0;text-align:center;">
  <div class="container" style="max-width:700px;">
    <h2 style="font-family:var(--fh);font-size:clamp(28px,5vw,52px);font-weight:900;letter-spacing:-0.02em;margin-bottom:18px;">${_esc(copy.ctaHeading||'Ready to get started?')}</h2>
    <p style="font-size:18px;opacity:.85;margin-bottom:36px;line-height:1.6;">${_esc(copy.ctaBody||'Contact us today and we\'ll get back to you within the hour.')}</p>
    <a href="tel:+1" class="btn" style="background:#fff;color:${p};font-size:17px;padding:17px 40px;">${_esc(copy.ctaCTA||'Call Now')}</a>
  </div>
</section>

<footer style="background:${s};color:#fff;padding:48px 0 32px;">
  <div class="container" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:24px;">
    <div>
      <div style="font-family:var(--fh);font-weight:900;font-size:20px;margin-bottom:6px;">${_esc(biz)}</div>
      <div style="font-size:13px;opacity:.5;">${_esc(copy.footerTagline||biz+' — '+ind)}</div>
    </div>
    <div style="display:flex;gap:24px;font-size:13px;opacity:.6;">
      <a href="#features" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.6">Services</a>
      <a href="#testimonials" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.6">Reviews</a>
      <a href="#cta" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.6">Contact</a>
    </div>
  </div>
  <div class="container" style="border-top:1px solid rgba(255,255,255,.08);margin-top:32px;padding-top:24px;font-size:12px;opacity:.35;text-align:center;">© ${new Date().getFullYear()} ${_esc(biz)}. All rights reserved.</div>
</footer>
...
function buildPreviewHTML(forExport = false) {
  ...
  const previewInteractScript = !forExport ? `
    <script>
      ...
      // ── SSV26.1 T4: Inline canvas text editing ──
      (function(){
        var EDITABLE = 'h1,h2,h3,h4,h5,h6,p,span,a,button,li,td,th,div,label';
        var activeEl = null;
        ...
        function activate(el) {
          if (activeEl && activeEl !== el) deactivate(activeEl, true);
          activeEl = el;
          el.setAttribute('contenteditable','true');
          el.dataset.ssOrig = el.textContent;
          el.style.outline = '2px solid rgba(255,107,53,0.8)';
        ...
        function deactivate(el, skipSave) {
          if (!el) return;
          ...
          if (!skipSave) {
            var orig = el.dataset.ssOrig || '';
            var now  = el.textContent;
            if (now !== orig) {
              var w = getWrapper(el);
              window.parent.postMessage({
                type:      'inlineEdit',
                blockId:   w ? w.dataset.blockId   : null,
                sectionId: w ? w.dataset.sectionId : null,
                original:  orig,
                newText:   now,
              }, '*');
            }
          }
          delete el.dataset.ssOrig;
          activeEl = null;
        }

        document.addEventListener('dblclick', function(e) {
          var t = e.target;
          if (t.closest('.ss-block-controls,.ss-sec-ctrl')) return;
          // Walk up to find an element with text directly (no deeply nested children)
          var el = t;
          while (el && el !== document.body) {
            var kids = Array.from(el.childNodes).filter(n => n.nodeType === 3 && n.textContent.trim());
            if (kids.length) break;
            var match = el.matches && el.matches(EDITABLE) ? el : null;
            if (match) break;
            el = el.parentElement;
          }
          if (!el || el === document.body) el = t.closest(EDITABLE);
          if (!el) return;
          e.preventDefault(); e.stopPropagation();
          activate(el);
        });
        ...
        document.addEventListener('blur', function(e) {
          if (e.target === activeEl) deactivate(e.target, false);
        }, true);

        document.addEventListener('keydown', function(e) {
          if (!activeEl) return;
          if (e.key === 'Escape') { activeEl.textContent = activeEl.dataset.ssOrig||activeEl.textContent; deactivate(activeEl, true); }
          if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); activeEl.blur(); }
        });
      })();
...
const SS_PASSWORD_DB = {
  ...
  agency: {
    ...
    'ROASRYE':     { note: 'Agency · ROASRYE access' },
    'GRAYSON2':    { note: 'Agency · Grayson access' },
    'POPPY2':      { note: 'Agency · Poppy access 2' },
    'SUPERSTUDIO1':{ note: 'Agency · Super Studio access' }
...
function openLoginModal(planHint) {
  // SSV27: auto-fill code from success page
  try {
    const prefill = sessionStorage.getItem('ss_prefill_code');
    if (prefill) {
      sessionStorage.removeItem('ss_prefill_code');
      setTimeout(() => {
...
function checkPassword() {
  ...
  if (result.ok) {
    ...
    // SSV26.7 — Remember-me persistence
    try {
...
function initApp() {
  ...
  // ── SSV28+: Restore SSections from session if available ──────
  if (loadFromSession() && State.sections && State.sections.length > 0) {
    refreshPreview();
    SSections.updatePanel();
    updateLayers();
    showToast('📂 Session restored', State.sections.length + ' custom sections loaded', 'info');
    return; // Skip onboarding — they have a site
  }

  // ── Check for SSV28 landing-page generated site (returning from checkout) ──
  try {
    ...
    if (saved) {
      ...
      if (site && site.html && Date.now() - site.savedAt < 4 * 60 * 60 * 1000) {
        ...
        if (SSections.load(site.html, site.prompt || '')) {
          ...
          return; // Skip onboarding — they already generated their site
        }
      }
    }
  } catch(e) {}
  ...
  // SSV26.7 — Always start in DESKTOP preview, never mobile.
  try {
  ...
  // SSV27: Always show onboarding on fresh login — session wipe means
...
/* ── Apply inline canvas text edit ───────────────────────────── */
function applyInlineEdit(msg) {
  const { blockId, sectionId, original, newText } = msg;
  if (!original || original === newText) return;

  if (sectionId && State.sections.length) {
    // SSections mode — direct HTML string replacement
    const sec = State.sections.find(s => s.id === sectionId);
    if (!sec) return;
    // Replace text node content carefully (not inside attribute values)
    sec.html = sec.html.replace(
      new RegExp('(>\\s*)' + _reEsc(original) + '(\\s*<)', 'g'),
      '$1' + newText.replace(/\$/g, '$$$$') + '$2'
    );
    History.push();
    showToast('✏️ Saved', 'Text updated', 'info');
    return;
  }

  if (blockId) {
    // Block mode — recursive string replace in block.data values
...
/* ── Nav link helpers (called from nav form inline) ─────────── */
    ...
    <div id="nav-content-panel" class="bsm-panel active">
      ...
      <div class="bsm-field">
        <label style="display:flex;align-items:center;justify-content:space-between;">Nav Links
          <button onclick="navLinkAdd()" style="background:rgba(255,107,53,0.15);border:1px solid rgba(255,107,53,0.3);color:#ff6b35;border-radius:5px;padding:3px 9px;font-size:11px;cursor:pointer;">＋ Add</button>
        </label>
        <div id="nav-links-list" style="display:flex;flex-direction:column;gap:6px;margin-top:6px;">
          ${(data.links||[]).map((l,i) => {
            ...
            return `<div style="display:grid;grid-template-columns:1fr 1fr auto;gap:4px;align-items:center;">
              ...
              <button onclick="navLinkRemove(${i})" style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;border-radius:5px;padding:5px 7px;font-size:11px;cursor:pointer;">✕</button>
            </div>`;
          }).join('')}
        </div>
      </div>
...
// Reliable features textarea handler — avoids inline IIFE scoping issues
...
function renderPhotoForm(data) {
  ...
  return `
      ...
      ${hasImg ? `
        <div style="width:100%;height:130px;border-radius:8px;overflow:hidden;margin-bottom:10px;position:relative;background:#0f0f18;">
          <img src="${data.imageUrl}" style="width:100%;height:100%;object-fit:cover;"
            onerror="this.parentElement.innerHTML='<div style=&quot;padding:20px;color:var(--ui-danger);font-size:12px;&quot;>⚠️ Image failed to load</div>'"/>
          ${data.credit ? `<div style="position:absolute;bottom:6px;right:6px;font-size:10px;color:#fff;background:rgba(0,0,0,0.5);padding:2px 7px;border-radius:4px;">📷 ${data.credit}</div>` : ''}
        </div>` : `
    ...
    <div class="bsm-field"><label>Or paste image URL</label>
      <input type="url" value="${data.imageUrl||''}" placeholder="https://images.unsplash.com/…"
    ...
    <div class="bsm-field"><label>Caption</label>
      <input type="text" value="${data.caption||''}" placeholder="Short description…"
    ...
    <div class="bsm-field"><label>Height</label>
      <select onchange="_editingData.height=this.value;livePreview()">
        <option value="300px" ${data.height==='300px'?'selected':''}>300px — Short</option>
        <option value="480px" ${(data.height||'480px')==='480px'?'selected':''}>480px — Medium</option>
        <option value="600px" ${data.height==='600px'?'selected':''}>600px — Tall</option>
        <option value="80vh" ${data.height==='80vh'?'selected':''}>80vh — Large</option>
        <option value="100vh" ${data.height==='100vh'?'selected':''}>100vh — Full screen</option>
      </select>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
    ...
    <div class="bsm-field"><label>Dark Overlay — ${opacityPct}%</label>
      <div class="slider-wrap">
        <input type="range" min="0" max="100" value="${opacityPct}"
          oninput="_editingData.overlayOpacity=this.value/100;this.previousElementSibling.textContent='Dark Overlay — '+this.value+'%';livePreview()"/>
    ...
    <div class="bsm-field"><label>Click Link (optional)</label>
      <input type="url" value="${data.linkUrl||''}" placeholder="https://…"
...
const UnplashLib = (function(){
  ...
  async function _search(query, page) {
    ...
    if (!grid) return;
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:48px 0;color:rgba(255,255,255,0.3);font-family:\'DM Sans\',sans-serif;font-size:14px;">Loading photos…</div>';
    ...
    try {
      ...
      if (!photos.length) {
        grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:48px 0;color:rgba(255,255,255,0.3);font-family:\'DM Sans\',sans-serif;font-size:14px;">No photos found — try a different search.</div>';
        return;
      }
...
/* ── MOBILE RESPONSIVE — full auto-optimization ── */
...
window.addEventListener('message', function(e) {
  ...
  switch (msg.type) {
    ...
    // SSections handlers
    case 'openSectionEditor':   openSectionEditor(msg.id); break;
...
function createNewProject() {
  // SSV26.7: window.prompt is disabled by default in Electron — use SSPrompt modal.
...
function deleteProject(name) {
  // SSV26.7: window.confirm is disabled in Electron — use SSConfirm modal.
  SSConfirm.ask({
...
const Onboarding = {
  ...
  _data: {
    ...
    competitors:    '',   // competitors they admire
    targetCustomer: '',   // who their ideal customer is
    vibes:          [],   // 3 brand personality words
    tone:           '',   // voice: professional/friendly/bold/luxury/playful
    palette:        '',
    goal:           '',
    cta:            '',   // CTA button text
    services:       '',   // services/products to highlight
    phone:          '',
    extra:          '',   // anything else for the AI
  },
  ...
  reset() {
    ...
    this._step = 0;
    this._data = { businessName:'', description:'', industry:'', location:'', differentiator:'', competitors:'', targetCustomer:'', vibes:[], tone:'', palette:'', goal:'', cta:'', services:'', phone:'', extra:'' };
    this._designs = [];
...
Make each design VISUALLY DISTINCT. Use different fonts, colors, border-radius, and layouts. The 3 designs should look nothing alike.`;
  ...
  /* ── Fallback designs if Gemini fails ─────────────────────── */
  _useFallbackDesigns() {
    const paletteCss = {
  ...
  /* ── Apply selected design to State ─── SSV27 FIX ────────── */
  _applyDesign(design) {
    const d = this._data;
    ...
    const goalCopy = {
      'Get people to call me':              { h:'Get Expert Help Fast', sub:'Serving ' + industry + ' — call us for a free quote today.', cta:'📞 Call Now', cta2:'Learn More' },
      'Get people to book an appointment':  { h:'Book Your Appointment', sub:'Fast, easy online booking for ' + name + '.', cta:'Book Now →', cta2:'See Availability' },
      ...
      'Build trust and credibility':        { h:'Trusted ' + industry + ' Experts', sub:name + ' — years of experience, hundreds of happy clients.', cta:'Meet the Team →', cta2:'Our Reviews' },
      'Sell products online':               { h:'Shop ' + name, sub:'Quality ' + industry + ' products — fast shipping, easy returns.', cta:'Shop Now →', cta2:'View All Products' },
    };
    const copy = goalCopy[goal] || { h:'Welcome to ' + name, sub:'Your trusted ' + industry + ' experts.', cta:'Get Started →', cta2:'Learn More' };
    ...
    order.forEach(type => {
      ...
      // Personalize each block type — prefer StarterCopy, fall back to generic
      if (type === 'nav') {
      ...
      if (type === 'features') {
        if (sc) {
          ...
          // features block uses data.items — not data.cards
          if (sc.items) {
          ...
          data.items = [
            { icon:'⚡', title:'Fast & Reliable',        description:'Quick turnaround with consistent, quality results every time.' },
            { icon:'🏆', title:'Proven Expertise',        description:'Years of hands-on experience in ' + industry + ' — local and trusted.' },
            { icon:'💰', title:'Transparent Pricing',     description:'Upfront quotes with zero hidden fees. You know what you pay before we start.' },
            { icon:'⭐', title:'5-Star Rated',             description:'Hundreds of happy customers and growing. Check our reviews.' },
            { icon:'📞', title:'Fast Response',            description:'We reply within the hour — no chasing, no waiting.' },
            { icon:'✅', title:'Satisfaction Guarantee',   description:'Not happy? We come back and make it right. No questions asked.' },
          ];
        }
      }
      ...
      if (type === 'cta') {
        if (sc) {
          ...
          data.subheading = 'Contact ' + name + ' today — we reply within the hour.';
          data.btnText    = copy.cta;
          data.btn2Text   = copy.cta2;
          data.bgColor    = primary;
          data.textColor  = '#ffffff';
      ...
      if (type === 'footer') {
        if (sc) {
          ...
          data.logo      = name;
          data.tagline   = name + ' — ' + industry + ' experts.';
          data.bgColor   = '#1a1a2e';
      ...
      if (type === 'leadform') {
        if (sc) {
          ...
          data.btnText    = sc.btnText || 'Get a Free Quote →';
    ...
    if (loading) {
      body.innerHTML = `
        <div class="ob-generating">
          ...
          <div class="ob-gen-title">Building your 3 designs…</div>
  ...
  /* ── BRAND INTERVIEW STEPS (SSV26.1 T2 redesign) ─────────────── */
  _steps: ['basics','location','customer','personality','direction','action','details'],
  // Total 7 interview steps then generate
  ...
  _renderProgress() {
    ...
    el.innerHTML = `
      ...
      <div style="position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:11px;color:rgba(255,255,255,0.35);font-family:'DM Sans',sans-serif;">${this._step + 1} / ${total}</div>`;
  },
  ...
  _renderStep() {
    ...
    const TONES = [
      { id:'professional', label:'Professional & Authoritative', desc:'Builds credibility. Good for lawyers, consultants, finance.' },
      { id:'friendly',     label:'Friendly & Warm',             desc:'Approachable and caring. Good for local services, health.' },
      { id:'bold',         label:'Bold & Confident',            desc:'Strong voice, direct. Good for gyms, trades, action-first brands.' },
      { id:'luxury',       label:'Luxury & Refined',            desc:'Premium feel. Good for spas, high-end retail, bespoke services.' },
      { id:'playful',      label:'Playful & Fun',               desc:'Light and engaging. Good for kids, food, entertainment.' },
    ];

    if (this._step === 0) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 1 · THE BASICS</div>
        <h2 class="ob-title">Let's start with your business</h2>
        <p class="ob-sub">This is your site's foundation — be specific and it'll change everything.</p>
        <label class="ob-field">
          <span class="ob-label">BUSINESS NAME</span>
          <input type="text" id="ob-biz-name" class="ob-input" placeholder="e.g. Luxe Studio, Mike's Plumbing" value="${this._esc(d.businessName)}" maxlength="60" autofocus>
        </label>
        <label class="ob-field">
          <span class="ob-label">WHAT DO YOU DO? <span style="opacity:.5;font-weight:400">(1-2 sentences)</span></span>
          <textarea id="ob-description" class="ob-textarea" rows="3" placeholder="e.g. We're an emergency plumbing service in Toronto. We specialise in burst pipes, blocked drains, and bathroom installations. Available 24/7.">${this._esc(d.description)}</textarea>
        </label>
        <label class="ob-field">
      ...
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 2 · YOUR MARKET</div>
        <h2 class="ob-title">Where are you based and who do you compete with?</h2>
        <p class="ob-sub">Local context makes your site feel real. Competitors help us understand the level you're playing at.</p>
        <label class="ob-field">
          <span class="ob-label">CITY / LOCATION</span>
          <input type="text" id="ob-location" class="ob-input" placeholder="e.g. Toronto, ON · East London · Miami Beach" value="${this._esc(d.location)}">
        </label>
        <label class="ob-field">
          <span class="ob-label">1-2 COMPETITORS YOU ADMIRE <span style="opacity:.5;font-weight:400">(optional)</span></span>
          <input type="text" id="ob-competitors" class="ob-input" placeholder="e.g. Dyno-Rod, local company you respect…" value="${this._esc(d.competitors)}">
          <span class="ob-hint">We use this to understand the aesthetic bar, not to copy them.</span>
        </label>`;

    } else if (this._step === 2) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 3 · YOUR PEOPLE</div>
        <h2 class="ob-title">Who is your ideal customer?</h2>
        <p class="ob-sub">Write like you're describing a real person. The more specific, the more the AI tailors your copy.</p>
        <label class="ob-field">
          <span class="ob-label">DESCRIBE YOUR IDEAL CUSTOMER</span>
          <textarea id="ob-customer" class="ob-textarea" rows="3" placeholder="e.g. Homeowners aged 30-55, usually panicking with a burst pipe at night. They need someone trustworthy fast — price matters but reliability matters more.">${this._esc(d.targetCustomer)}</textarea>
        </label>
        <label class="ob-field">
          <span class="ob-label">WHAT MAKES YOU DIFFERENT FROM COMPETITORS?</span>
          <textarea id="ob-differentiator" class="ob-textarea" rows="2" placeholder="e.g. We're the only team in Toronto that guarantees arrival in 90 minutes, 24/7, including holidays.">${this._esc(d.differentiator)}</textarea>
        </label>`;

    } else if (this._step === 3) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 4 · BRAND PERSONALITY</div>
        <h2 class="ob-title">Pick up to 3 words that describe your brand</h2>
        <p class="ob-sub">These drive font choices, spacing, and the overall feel of your site.</p>
        <div class="ob-chip-grid" id="ob-vibes">
        ...
        <div class="ob-chip-hint">${d.vibes.length > 0 ? d.vibes.join(' · ') : 'Pick up to 3'}</div>`;

    } else if (this._step === 4) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 5 · VOICE & DIRECTION</div>
        <h2 class="ob-title">How does your brand sound and look?</h2>
        <p class="ob-sub">Tone shapes every word the AI writes. Palette is just a direction — AI picks the exact colours.</p>
        <label class="ob-field">
          <span class="ob-label">TONE OF VOICE</span>
          <div class="ob-radio-list" style="margin-top:6px;">
            ${TONES.map(t => `
              <label class="ob-radio ${d.tone===t.id?'selected':''}" style="flex-direction:column;align-items:flex-start;gap:2px;padding:10px 14px;">
                <div style="display:flex;align-items:center;gap:8px;width:100%;">
                  <input type="radio" name="ob-tone" value="${t.id}" ${d.tone===t.id?'checked':''} onchange="Onboarding._data.tone=this.value;Onboarding._highlightRadio(this)">
                  <span style="font-weight:600;">${t.label}</span>
                </div>
                <span style="font-size:12px;opacity:.55;padding-left:20px;">${t.desc}</span>
              </label>`).join('')}
          </div>
        </label>
        <label class="ob-field" style="margin-top:14px;">
          <span class="ob-label">COLOUR DIRECTION</span>
          <div class="ob-palette-grid" style="margin-top:8px;">
            ${this.PALETTES.map(p => `
              <button type="button" class="ob-palette-card ${d.palette===p.id?'selected':''}" onclick="Onboarding._selectPalette('${p.id}')">
                <div class="ob-swatches">${p.preview.map(c => `<div class="ob-swatch" style="background:${c}"></div>`).join('')}</div>
                <div class="ob-palette-label">${p.label}</div>
              </button>`).join('')}
          </div>
        </label>`;
      ...
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 6 · YOUR CALL TO ACTION</div>
        <h2 class="ob-title">What should visitors do when they land on your site?</h2>
        <p class="ob-sub">This becomes your hero button, CTA section, and nav link.</p>
        <label class="ob-field">
          <span class="ob-label">PRIMARY GOAL</span>
          <div class="ob-radio-list" style="margin-top:6px;">
            ${this.GOALS.map(g => `
              <label class="ob-radio ${d.goal===g?'selected':''}">
                <input type="radio" name="ob-goal" value="${g}" ${d.goal===g?'checked':''} onchange="Onboarding._data.goal=this.value;Onboarding._highlightRadio(this)">
                <span>${g}</span>
              </label>`).join('')}
          </div>
        </label>
        <label class="ob-field">
          <span class="ob-label">BUTTON TEXT <span style="opacity:.5;font-weight:400">(optional)</span></span>
          <input type="text" id="ob-cta" class="ob-input" placeholder="e.g. Get a Free Quote · Book Now · Call Us Today" value="${this._esc(d.cta)}">
        </label>
        <label class="ob-field">
          <span class="ob-label">SERVICES / PRODUCTS TO HIGHLIGHT <span style="opacity:.5;font-weight:400">(optional)</span></span>
          <textarea id="ob-services" class="ob-textarea" rows="2" placeholder="e.g. Emergency callouts, drain unblocking, boiler installs. Fixed pricing from £90.">${this._esc(d.services)}</textarea>
        </label>`;

    } else if (this._step === 6) {
      body.innerHTML = `
        <div class="ob-eyebrow">STEP 7 · FINAL DETAILS</div>
        <h2 class="ob-title">Almost done — a couple more things</h2>
        <p class="ob-sub">These go directly into your site. Both are optional.</p>
        <label class="ob-field">
          <span class="ob-label">PHONE NUMBER <span style="opacity:.5;font-weight:400">(optional)</span></span>
          <input type="tel" id="ob-phone" class="ob-input" placeholder="e.g. +1 (416) 555-0100" value="${this._esc(d.phone)}">
        </label>
        <label class="ob-field">
          <span class="ob-label">ANYTHING ELSE FOR THE AI? <span style="opacity:.5;font-weight:400">(optional)</span></span>
          <textarea id="ob-extra" class="ob-textarea" rows="3" placeholder="e.g. We want the site to feel premium but approachable. Use photography of real tradespeople, not stock suits. Avoid clichés like 'we go above and beyond'.">${this._esc(d.extra)}</textarea>
        </label>`;
    }
    ...
    foot.innerHTML = `
      <div class="ob-foot-left">
        ${this._step > 0 ? '<button type="button" class="ob-btn ob-btn-ghost" onclick="Onboarding.back()">← Back</button>' : ''}
        <button type="button" class="ob-btn ob-btn-skip" onclick="Onboarding.close(true)">Skip</button>
      </div>
      <div class="ob-foot-right">
        <button type="button" class="ob-btn ob-btn-primary" onclick="Onboarding.next()">
          ${isLast ? '✨ Build my site →' : 'Next →'}
        </button>
      </div>`;
  },
  ...
  _renderDesignPicker() {
    ...
    m.innerHTML = `
      <div class="ob-card ob-card-wide" id="ob-card">
        ...
        <p class="ob-sub" style="text-align:center;padding:0 24px 20px;">Each is unique — fonts, colors, layout, and photos are all different.</p>
        ...
        <div class="ob-design-foot">
          <button type="button" class="ob-btn ob-btn-ghost" onclick="Onboarding._regenerate()">🔄 Generate new designs</button>
  ...
  _confirmDesign() {
    ...
    showToast('✨ ' + (this._data.businessName || 'Your site') + ' is ready', 'AI built your site — click any block to edit it.', 'success');
  },
  ...
  next() {
    ...
    if (this._step === 0) {
      ...
      if (!name) { this._shake(); showToast('⚠️ Business name required', '', 'warning'); return; }
      if (!desc) { this._shake(); showToast('⚠️ Tell us what you do', 'A couple of sentences helps a lot.', 'warning'); return; }
      if (!ind)  { this._shake(); showToast('⚠️ Pick your industry', '', 'warning'); return; }
      d.businessName = name; d.description = desc; d.industry = ind;
      State.businessName = name; State.industry = ind;

    } else if (this._step === 1) {
      d.location    = document.getElementById('ob-location')?.value?.trim()    || '';
      d.competitors = document.getElementById('ob-competitors')?.value?.trim() || '';

    } else if (this._step === 2) {
      const cust = document.getElementById('ob-customer')?.value?.trim();
      const diff = document.getElementById('ob-differentiator')?.value?.trim();
      if (!cust) { this._shake(); showToast('⚠️ Describe your ideal customer', 'Even one sentence makes a big difference.', 'warning'); return; }
      d.targetCustomer = cust; d.differentiator = diff || '';

    } else if (this._step === 3) {
      if (!d.vibes.length) { showToast('⚠️ Pick at least one word', '', 'warning'); return; }

    } else if (this._step === 4) {
      if (!d.tone)    { this._shake(); showToast('⚠️ Pick a tone of voice', '', 'warning'); return; }
      if (!d.palette) { this._shake(); showToast('⚠️ Pick a colour direction', '', 'warning'); return; }

    } else if (this._step === 5) {
      if (!d.goal) { this._shake(); showToast('⚠️ Pick your primary goal', '', 'warning'); return; }
      d.cta      = document.getElementById('ob-cta')?.value?.trim()      || '';
      d.services = document.getElementById('ob-services')?.value?.trim() || '';

    } else if (this._step === 6) {
      // Last step — collect and generate
    ...
    reader.onload = e => {
      ...
      if (drop) {
        drop.innerHTML = `<img src="${e.target.result}" style="max-height:80px;max-width:200px;object-fit:contain;border-radius:8px;" alt="logo">
          <div class="ob-logo-sub" style="margin-top:8px;">Logo uploaded ✓</div>`;
      }
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   ────────────────────────────────────────────────────────────────────────
   F10: AIBridge       — talks to OpenAI GPT-5 from browser
...
/* ───────────────────── F10: AI BRIDGE (SSV27 — NVIDIA NIM) ───────────────────── */
const AIBridge = {
  ...
  async chat({ messages, tools, model, signal }) {
    ...
    if (tools && tools.length) {
      // Pollinations doesn't support native tool calling — inject as JSON protocol
...
const AIChatbot = {
  ...
  async send(userText) {
    ...
    try {
      ...
      let m = '⚠️ ' + err.message;
      if (err.message === 'NO_API_KEY')      m = '🔑 Add your OpenAI GPT-5 key (click the key icon).';
      ...
      if (err.message === 'PAYMENT_REQUIRED')m = '💳 OpenAI credits exhausted or quota hit. Check your billing at platform.openai.com.';
...
(function patchOnboarding(){
  ...
  Onboarding.finish = async function() {
    ...
    // SSV26.7+: built-in keyless GPT-5 gateway is always on — no key prompt needed.
...
(function patchBridge(){
  ...
  // SSV26.7 — built-in GPT-5 gateway (no key required)
    ...
    if (tools && tools.length) {
      ...
      body.messages = [{
        ...
        content:
          ...
          'Available tools:\n' + toolsList + '\n\n' +
          'Rules: raw JSON only — no markdown fences.'
  ...
  AIBridge.chat = async function({ messages, tools, model, signal, jsonOnly }) {
    // 1) If user supplied an OpenAI key → use real OpenAI GPT-5
    if (this.getKey()) {
...
/* SSV28+ — Onboarding generates 100% custom HTML per user via AI pipeline.
   No shared BlockRenderers. Every site is unique from the foundation. */
(function patchOnboardingV264(){
  ...
  Onboarding.finish = async function() {
    ...
    showToast('🤖 Generating your site…', 'AI is building a unique site for ' + (d.businessName||'your business'), 'info');

    try {
      const html = await generateCustomSiteHTML({
        businessName:   d.businessName   || 'Your Business',
        businessType:   d.industry       || d.businessType || 'service business',
      ...
      if (html && SSections.load(html, d.businessName)) return;
      throw new Error('empty html');
    } catch(err) {
      console.warn('[SSV28 onboarding gen failed]', err);
      showToast('⚠️ Generation failed', 'Using template fallback — you can still edit.', 'error');
      // Fallback: use old block system
...
/* ═══════════════════════════════════════════════════════════════════
   SSV26.9 — FUNCTIONAL AI EDIT REPAIR
...
(function ssv269FunctionalAIEditRepair(){
  ...
  const Intent = {
    parse(text) {
      ...
      if (!t) return null;
      let m;
      m = raw.match(/(?:change|update|set|make)\s+(?:the\s+)?(?:hero\s+)?(headline|title|heading|subheading|subtitle|sub headline|button text|cta|logo|tagline)\s+(?:to|=)\s+["“']?([^"”'\n]+)["”']?\s*$/i);
      if (m) {
    ...
    async execute(intent) {
      ...
      if (intent.name === 'recolor') {
        ...
        return '✓ Updated the site colors.';
      }
      if (intent.name === 'generate_site') {
        if (typeof AISiteGenerator !== 'undefined' && AISiteGenerator.run) {
          ...
          const ok = await AISiteGenerator.run(info);
          return ok ? '✓ Regenerated the site with a new style.' : '⚠️ I could not regenerate it right now.';
        }
        return '⚠️ Site generator is unavailable.';
      }
      const result = await AIChatbot.runTool(intent.name, intent.args || {});
      if (result && result.error) return '⚠️ ' + result.error;
      if (intent.name === 'edit_block_text') return '✓ Updated the site text.';
      if (intent.name === 'add_block') return '✓ Added the ' + normalizeType(intent.args.type) + ' section.';
      if (intent.name === 'remove_block') return '✓ Removed the section.';
      if (intent.name === 'set_palette') return '✓ Updated the site colors.';
      if (intent.name === 'fetch_images') return '✓ Updated the hero image.';
      return '✓ Done.';
    }
  };
  window.SSIntent = Intent;
  ...
  AIChatbot.send = async function(userText) {
    ...
    const finish = (text) => {
      ...
      this.appendMsg('assistant', text || '✓ Done.');
      this._history.push({ role:'assistant', content:text || '✓ Done.' });
    };
    try {
      ...
      if (plan && Array.isArray(plan.actions) && plan.actions.length) {
        ...
        finish(plan.say || '✓ Updated the site.');
        return;
      }

      if (local) {
        this.appendThinking('⚙ local edit');
        finish(await Intent.execute(local));
        return;
      }

      finish((plan && plan.say) || raw.replace(/```[a-z]*|```/gi, '').trim().slice(0, 500) || 'Tell me the exact edit you want, like “change the headline to Fresh Cuts” or “add a pricing section.”');
    } catch(err) {
      console.warn('[SSV26.9 AIChatbot.send]', err);
      const local = Intent.parse(userText);
      if (local) finish(await Intent.execute(local));
      else finish('⚠️ ' + (err.message || 'The edit failed.'));
    } finally {
      this._busy = false;
      this.setBusy(false);
    }
...
/* ═══════════════════════════════════════════════════════════════════
   SSV26.7 — DESKTOP-FRIENDLY DIALOGS (replace window.prompt / confirm)
...
/* ═══════════════════════════════════════════════════════════════════
   SSV26.7 — AI IMPROVER (returning-user experience)
  ...
  _showPanel(suggestions) {
    ...
    const items = suggestions.improvements.map((s, i) =>
      '<li class="ai-improve-item">' +
        ...
        (s.value ? '<div class="ai-improve-value">→ ' + s.value + '</div>' : '') +
    ...
    node.innerHTML =
      '<div class="ss-dialog-card ai-improve-card" role="dialog" aria-modal="true">' +
        '<div class="ss-dialog-title">✨ AI improvements for your site</div>' +
    ...
    node.querySelector('.ai-improve-apply-all').onclick = () => {
      ...
      close();
      if (typeof showToast === 'function') showToast('✨ Improvements applied', 'AI updated your site', 'success');
    };
  },
...
/* ════════════════════════════════════════════════════════════════════
   SSV26.8 — NEW FEATURES BUNDLE
   1) EmailAuth     — local email + password accounts (per-device)
   2) StudioMode    — agency-tier dual-pane Developer + AI Agent surface
   3) Bug fixes & quality-of-life patches
   ════════════════════════════════════════════════════════════════════ */

/* ───────────── 1. EMAIL + PASSWORD AUTH (local, per-device) ─────────────
...
const EmailAuth = {
  ...
  saveSession(email, token, remember) {
    // SSV27: no persistent sessions — always sessionStorage regardless of remember
  ...
  setMode(mode) {
    ...
    if (submit) submit.textContent = (this._mode === 'signup') ? 'Create Account →' : 'Sign In →';
    ...
    try {
      if (this._mode === 'signup') {
        ...
        if (bindCode) {
          ...
          if (!r.ok) { if (btn) { btn.textContent = 'Sign Up →'; btn.disabled = false; } return this._fail('Invalid plan code. Leave blank for Free.'); }
          tier = r.tier;
        }
        ...
        if (typeof showToast === 'function') showToast('🎉 Account created', 'Welcome to Supersuite!', 'success');
      ...
      if (btn) { btn.textContent = this._mode === 'signup' ? 'Sign Up →' : 'Sign In →'; btn.disabled = false; }
  ...
  _activate(acc, silent) {
    ...
    if (!silent && typeof showToast === 'function') {
      showToast('👋 Welcome back', acc.name || acc.email, 'success');
    }
    return true;
  },
  ...
  forgot() {
    ...
    this._hash(newPwd, acc.salt).then(h => {
      ...
      if (typeof showToast === 'function') showToast('🔐 Password reset', 'You can now sign in with the new password.', 'success');
    });
  },
...
/* ════════════════════════════════════════════════════════════════════
   2. STUDIO MODE — Agency-tier only · Beta
   Two routes inside one panel:
   • Developer   — read/edit live HTML/CSS/JS templates + see preview source
   • AI Agent    — autonomous agent with tool access (edit blocks, fetch
...
const StudioMode = {
  ...
  ensureUI() {
    ...
    if (navRight && !document.getElementById('studio-launch-btn')) {
      ...
      btn.title = 'Studio Mode (Agency · Beta)';
      btn.innerHTML = '🧪 Studio';
    ...
    root.innerHTML = `
      ...
      <aside class="studio-panel">
        <header class="studio-header">
          ...
          <button class="studio-close" onclick="StudioMode.toggle(false)" aria-label="Close">✕</button>
        </header>
        ...
        <section class="studio-body" id="studio-body-developer">
          <div class="studio-dev-tabs">
            ...
            <button class="sdt-action" onclick="StudioMode.refreshDev()" title="Re-read current preview">↻ Refresh</button>
            <button class="sdt-action primary" onclick="StudioMode.applyDev()" title="Apply edits to live preview">✓ Apply</button>
          </div>
          <div class="studio-dev-views">
            ...
            <div class="sdv" id="sdv-html"><textarea spellcheck="false" id="studio-html-area" placeholder="HTML source…"></textarea></div>
            ...
            <div class="sdv" id="sdv-js"><textarea spellcheck="false" id="studio-js-area" placeholder="JS console (read-only — use AI Agent to run code)"></textarea></div>
            <div class="sdv" id="sdv-state"><textarea spellcheck="false" id="studio-state-area" placeholder="Live state JSON…"></textarea></div>
          </div>
          <div class="studio-dev-warn">
            ⚠️ Beta · HTML edits write to State.customCSS / blocks where possible. JS is read-only — use the AI Agent to mutate state.
          </div>
        </section>
          ...
          <div class="studio-agent-shell">
            <div class="studio-agent-stream" id="studio-agent-stream">
              <div class="sas-msg sas-system">
                <strong>🤖 Studio Agent · Beta</strong><br/>
            ...
            <div class="studio-agent-tools">
              <details>
                <summary>🔧 API Endpoints (the agent can call these)</summary>
                <div class="sat-config">
                  ...
                  <button class="sat-add-btn" onclick="StudioMode.addApi()">＋ Add Endpoint</button>
                </div>
              </details>
              <details>
            ...
            <form class="studio-agent-input" id="studio-agent-form" onsubmit="event.preventDefault(); StudioMode.agentSend();">
              <textarea id="studio-agent-text" rows="2" placeholder="Tell the agent what to do… (Shift+Enter for newline)"></textarea>
              <div class="sai-row">
                <button type="button" class="sai-clear" onclick="StudioMode.agentClear()">Clear</button>
                <button type="submit" class="sai-send" id="studio-agent-send">Run →</button>
              </div>
            </form>
          </div>
        </section>
  ...
  toggle(force) {
    if (!this.isAvailable()) {
      if (typeof showToast === 'function')
        showToast('🔒 Studio Mode is Agency-only', 'Upgrade to Agency tier to unlock the developer + AI agent surface.', 'warn');
      return;
    }
    this.ensureUI();
    this._open = (force === undefined) ? !this._open : !!force;
  ...
  applyDev() {
    try {
      ...
      if (stateA && stateA.value.trim()) {
        ...
        } catch(err) { return showToast('❌ Invalid JSON', err.message, 'warn'); }
      }
      if (typeof saveHistory === 'function') saveHistory();
      if (typeof refreshPreview === 'function') refreshPreview();
      this.refreshDev();
      if (typeof showToast === 'function') showToast('✓ Applied', 'Studio edits pushed to live preview.', 'success');
    } catch(e) { showToast('❌ Apply failed', e.message, 'warn'); }
  },
  ...
  renderApis() {
    ...
    list.innerHTML = apis.map((a, i) => `
      <div class="sat-api-row">
        ...
        <input type="text" placeholder="https://api…  (or paste API key)" value="${(a.url||'').replace(/"/g,'&quot;')}" oninput="StudioMode.updateApi(${i},'url',this.value)" onblur="StudioMode._autodetect(${i})"/>
        <input type="password" placeholder="API key (Bearer)" value="${(a.key||'').replace(/"/g,'&quot;')}" oninput="StudioMode.updateApi(${i},'key',this.value)"/>
        <button class="sat-del" onclick="StudioMode.removeApi(${i})">✕</button>
  ...
  agentClear() {
    ...
    if (s) s.innerHTML = '<div class="sas-msg sas-system">🤖 Cleared. Ready for a new task.</div>';
  },
  ...
  async agentSend() {
    ...
    if (this._agentMessages.length === 1) {
      this._agentMessages.unshift({
        ...
        content:
          ...
          'Use the provided tools — do not write code in plain text. Prefer concrete actions. ' +
    ...
    const status = this._appendMsg('thinking', '🤖 Thinking…');
    ...
    try {
      ...
      while (safety++ < 6) {
        ...
        for (const c of calls) {
          ...
          this._appendMsg('tool-call', '<code>→ ' + c.function.name + '(' + this._briefArgs(args) + ')</code>');
          const result = await this._runTool(c.function.name, args);
          this._appendMsg('tool-result', '<code>← ' + this._briefResult(result) + '</code>');
...
/* ════════════════════════════════════════════════════════════════════
   SSV26.9 — STANDALONE SS STUDIO AGENT
   ────────────────────────────────────────────────────────────────────
...
const StudioAgentApp = {
  ...
  // ── Tools the standalone agent can call ────────────────────────
  _tools: [
  ...
  clear() {
    ...
    if (s) s.innerHTML = '<div class="sa-msg sa-msg-system"><strong>🤖 Studio Agent</strong> cleared. Ready for a new task.</div>';
  },

  // ── API endpoints (separate store from StudioMode) ────────────
  _readApis() { try { return JSON.parse(localStorage.getItem(this._apiKey) || '[]'); } catch(e) { return []; } },
  _writeApis(arr) { try { localStorage.setItem(this._apiKey, JSON.stringify(arr)); } catch(e) {} },
  renderApis() {
    ...
    list.innerHTML = apis.map((a, i) => `
      <div class="sa-api-row">
        ...
        <input type="text" placeholder="https://api…  (or paste API key)" value="${(a.url||'').replace(/"/g,'&quot;')}" oninput="StudioAgentApp.updateApi(${i},'url',this.value)" onblur="StudioAgentApp._autodetect(${i})"/>
        <input type="password" placeholder="API key (Bearer)" value="${(a.key||'').replace(/"/g,'&quot;')}" oninput="StudioAgentApp.updateApi(${i},'key',this.value)"/>
        <button title="Remove" onclick="StudioAgentApp.removeApi(${i})">✕</button>
      </div>`).join('') || '<p class="sa-empty">No endpoints yet. Paste a URL <em>or</em> just an API key — the agent figures it out.</p>';
  },
  addApi() { const arr = this._readApis(); arr.push({ label: '', url: '', key: '' }); this._writeApis(arr); this.renderApis(); },
  removeApi(i) { const arr = this._readApis(); arr.splice(i, 1); this._writeApis(arr); this.renderApis(); },
  updateApi(i, k, v) { const arr = this._readApis(); if (arr[i]) { arr[i][k] = v; this._writeApis(arr); } },
  ...
  // ── Hand off to the builder (and Studio Mode if available) ────
  openBuilderHandoff() {
  ...
  async send() {
    ...
    if (this._messages.length === 1 || this._messages[0].role !== 'system') {
      this._messages.unshift({
        role: 'system',
        content:
          'You are SS Studio Agent — a standalone, agency-tier AI agent for Supersuite. ' +
...
/* ── Gate handlers (landing-page wiring) ─────────────────────── */
...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Single floating button → expandable side panel.
   Pollinations AI interprets natural language → edits the live site.
...
const AIHub = {
  ...
  _getKey() { return ''; }, // keyless via Pollinations
  ...
  mount() {
    ...
    fab.title = 'AI Editor';
    fab.innerHTML = '<span class="aihub-fab-icon">✦</span><span class="aihub-fab-label">AI Edit</span>';
    ...
    panel.innerHTML = `
      <div class="aihub-head">
        <div>
          <div class="aihub-eyebrow">AI EDITOR · Gemini Flash</div>
          <div class="aihub-title">Edit your site</div>
        </div>
        <button class="aihub-close" onclick="AIHub.toggle(false)">✕</button>
      </div>
      <div class="aihub-messages" id="aihub-messages">
        <div class="aihub-welcome">
          <div class="aihub-welcome-icon">✦</div>
          <p>Tell me what to change. Try:</p>
          <div class="aihub-suggestions">
            <button onclick="AIHub._sendSuggestion(this.textContent)">Make the hero darker</button>
            <button onclick="AIHub._sendSuggestion(this.textContent)">Rewrite the headline</button>
            <button onclick="AIHub._sendSuggestion(this.textContent)">Add a testimonials section</button>
            <button onclick="AIHub._sendSuggestion(this.textContent)">Make buttons more rounded</button>
          </div>
        </div>
      </div>
      <div class="aihub-input-area">
        <div class="aihub-input-row">
          <textarea id="aihub-input" class="aihub-textarea" placeholder="Tell me what to change…" rows="2" onkeydown="AIHub._onKey(event)"></textarea>
          <button class="aihub-send" id="aihub-send" onclick="AIHub.send()">→</button>
        </div>
        <div class="aihub-foot-row">
          <button class="aihub-undo" onclick="AIHub._undo()">↩ Undo last</button>
          <span class="aihub-model">Pollinations AI</span>
        </div>
      </div>`;
  ...
  _setLoading(v) {
    ...
    const inp = document.getElementById('aihub-input');
    if (btn) { btn.disabled = v; btn.textContent = v ? '…' : '→'; }
  ...
  _buildPrompt(userText) {
    const siteSnap = {
      ...
      colors:       {
        ...
        secondary: State.globalStyles['--secondary'],
        accent:    State.globalStyles['--accent'],
        bg:        State.globalStyles['--bg'],
        text:      State.globalStyles['--text']
      },
      fonts: {
        heading: State.globalStyles['--font-heading'],
        body:    State.globalStyles['--font-body']
      },
      radius:  State.globalStyles['--radius'],
      customCSS: (State.customCSS || '').slice(0, 500)
    };

    return `You are an AI web editor for Supersuite, a no-code website builder.
The user's current site state:
${JSON.stringify(siteSnap, null, 2)}

The user says: "${userText}"

Respond with a JSON object (no markdown fences) with these optional fields:
{
  "reply": "friendly 1-sentence confirmation of what you did",
  "cssVars": { "--primary": "#...", "--secondary": "...", ... },
  "customCSS": "additional CSS to inject (or null)",
  "blockEdits": [
    { "type": "hero", "data": { "heading": "new headline", "sub": "new subhead" } }
  ],
  "addBlock": "block_type_to_add or null",
  "removeBlock": "block_type_to_remove or null",
  "unsplashQuery": "search query for new images or null"
}

Only include fields that need changing. Keep changes minimal and correct.`;
  },
  ...
  _undo() {
    if (!this._lastStateSnapshot) {
      this._addMessage('assistant', 'Nothing to undo yet.');
      return;
    }
    try {
      const snap = JSON.parse(this._lastStateSnapshot);
      ...
      if (typeof refreshPreview === 'function') refreshPreview();
      this._addMessage('assistant', 'Undone ✓ Reverted to your previous state.');
      this._lastStateSnapshot = null;
    } catch(e) {
      this._addMessage('assistant', 'Undo failed: ' + e.message);
    }
  },
  ...
  async send() {
    ...
    // Save undo snapshot
    this._lastStateSnapshot = JSON.stringify({
      globalStyles: { ...State.globalStyles },
      customCSS: State.customCSS,
    ...
    try {
      const res = await fetch(this.POLL_URL, {
        ...
        body: JSON.stringify({
          model:   this.POLL_MODEL,
          messages:[{ role: 'user', content: this._buildPrompt(text) }],
          private: true,
          referrer:'supersuite',
          jsonMode: true
        })
      ...
      let json; try { json = JSON.parse(rawText); } catch(e) { json = {choices:[{message:{content:rawText}}]}; }
      ...
      try { parsed = JSON.parse(clean); } catch(e) {
        ...
        parsed = match ? JSON.parse(match[0]) : { reply: clean };
      }
      this._applyEdit(parsed);
      this._addMessage('assistant', this._esc(parsed.reply || 'Done ✓'));
    } catch(err) {
  ...
  _applyEdit(edit) {
    if (!edit) return;
    // CSS variables
    if (edit.cssVars && typeof edit.cssVars === 'object') {
      Object.entries(edit.cssVars).forEach(([k,v]) => {
        State.globalStyles[k] = v;
        document.documentElement.style.setProperty(k, v);
      });
    }
    ...
    // Block content edits
    if (edit.blockEdits && Array.isArray(edit.blockEdits)) {
      edit.blockEdits.forEach(be => {
        const block = State.blocks && State.blocks.find(b => b.type === be.type);
        if (block && be.data) {
          Object.assign(block.data, be.data);
        }
      });
    }
    // Add block
    if (edit.addBlock && typeof addBlock === 'function') {
      try { addBlock(edit.addBlock); } catch(e) {}
    }
    // Unsplash
    if (edit.unsplashQuery && typeof Unsplash !== 'undefined') {
      Unsplash.applyToBlocks(edit.unsplashQuery).catch(() => {});
    }
    if (typeof refreshPreview === 'function') refreshPreview();
  },
...
/* ═══════════════════════════════════════════════════════════════════
   SSV27 — MOBILE DETECTION + ONBOARDING-ONLY MOBILE FLOW
  ...
  function injectMobileBanner(){
    ...
    b.innerHTML = `
      <div class="smb-inner">
        ...
        <div class="smb-text">
          <strong>Mobile mode</strong> — onboarding and AI edits only.
          <span>Open Supersuite on a desktop for the full builder.</span>
        </div>
        <button class="smb-close" onclick="this.closest('#ss-mobile-banner').remove()">✕</button>
      </div>`;
    ...
    if (canvas) {
      ...
      msg.innerHTML = `
        <div class="smcm-inner">
          ...
          <p style="margin-top:16px;opacity:.7;font-size:13px;">You can still make AI edits below ↓</p>
        </div>`;
...
/* ═══════════════════════════════════════════════════════════════════
   SSV27 — POST-EXPORT LAUNCH PAGE
   After export, opens a full-screen launch experience:
     • Live preview of exported site
     • Deploy to supersuite.app subdomain (one click)
     • Connect own Vercel account
     • Custom domain setup
═══════════════════════════════════════════════════════════════════ */
...
const SSLaunch = {
          ...
          <div class="sslp-options-col">
            ...
            <div class="sslp-option" id="sslp-opt-subdomain">
              <div class="sslp-option-head">
                <div class="sslp-option-icon">🌐</div>
                <div>
                ...
                <div class="sslp-option-badge">$3–5/mo</div>
              </div>
            ...
            <div class="sslp-option">
              <div class="sslp-option-head">
                ...
                <div>
                  ...
                  <div class="sslp-option-sub">Deploy to your Vercel account — free on their hobby plan</div>
                </div>
            ...
            <div class="sslp-option sslp-option-plain">
              <div class="sslp-option-head">
                ...
                <div>
                  ...
                  <div class="sslp-option-sub">Host the HTML/CSS/JS files anywhere — cPanel, Netlify, GitHub Pages, etc.</div>
                </div>
              </div>
  ...
  _renderPreview() {
    ...
    try {
      ...
      if (frame) frame.innerHTML = '<div class="sslp-preview-error">Preview unavailable — files downloaded successfully.</div>';
    }
  },
  ...
  async _deploySubdomain() {
    ...
    this._setStatus(status, 'loading', '🚀 Deploying to ' + slug + '.supersuite.app…');
    ...
    this._setStatus(status, 'success',
      `✓ Ready! Your site will be live at <a href="https://${slug}.supersuite.app" target="_blank">${slug}.supersuite.app</a> once connected.
       <br><small style="opacity:.7;margin-top:4px;display:block;">Full deployment connects via Vercel — coming in Phase 3.</small>`);
  },
  ...
  async _deployVercel() {
    ...
    this._setStatus(status, 'loading', '▲ Uploading to Vercel…');
    try {
      const html = typeof buildExportHTML === 'function' ? buildExportHTML() : '';
      ...
      this._setStatus(status, 'success', `✓ Deployed! <a href="${url}" target="_blank">${url}</a>`);
    } catch(e) {
