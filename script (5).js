...
const LiveSync = {
  ...
  // Use timestamps (not sequential versions) to avoid cross-tab counter divergence
  _lastAppliedTs: 0,     // timestamp of last state we applied from a peer
  _pendingPushTs: 0,     // timestamp of our last outgoing push
  _syncInProgress: false, // guard: prevents echo loop when applying remote state
  _syncTimeoutId: null,   // for stuck-syncing recovery
  ...
  init(code, isOwner) {
    ...
    try {
      this.channel = new BroadcastChannel('supersuite_collab_' + code);
    } catch(e) {
  ...
  /** Serialize and broadcast the full current state to all peers in this session */
  push() {
    if (!this.channel) return;
    // Do not echo back a state we just received from a peer
    if (this._syncInProgress) return;

    const ts = Date.now();
    this._pendingPushTs = ts;

    // Deep-clone state to avoid sending live references
    let stateSnapshot;
    try {
    ...
    try {
      ...
      console.error('[LiveSync] postMessage failed:', e);
      this._flashSyncIndicator('error');
      return;
    }

    // Show syncing indicator on sender side, then auto-resolve after 1s
    // (BroadcastChannel does not echo back to sender, so we resolve locally)
    this._flashSyncIndicator('syncing');
    clearTimeout(this._syncTimeoutId);
    this._syncTimeoutId = setTimeout(() => {
      // If we haven't received a newer remote state, resolve to 'synced'
  ...
  _onMessage(data) {
    ...
    // Handle sync request: a new joiner is requesting full state from owner
    if (data.type === 'REQUEST_SYNC') {
      // Anyone with a channel can respond (last writer wins anyway)
      console.log('[LiveSync] Peer requested full sync, pushing state');
      // Small delay so requester's channel is fully set up
    ...
    // Ignore stale updates (older than the last one we applied)
    // Use a small tolerance (50ms) to handle clock skew
    if (data.ts <= this._lastAppliedTs - 50) {
      console.log('[LiveSync] Ignoring stale update ts=' + data.ts + ' lastApplied=' + this._lastAppliedTs);
      return;
    }

    // Guard: prevent applying our own echo (BroadcastChannel doesn't self-echo
    // in spec, but guard anyway)
    if (data.ts === this._pendingPushTs && data.sender === (this.isOwner ? 'owner' : 'guest')) {
      return;
    }
    ...
    try {
      ...
      // Full authoritative replace — remote state is the truth
      if (s.blocks !== undefined) {
        State.blocks = JSON.parse(JSON.stringify(s.blocks));
      }
      if (s.globalStyles !== undefined) {
        State.globalStyles = JSON.parse(JSON.stringify(s.globalStyles));
      }
      if (s.currentTemplate !== undefined) {
        State.currentTemplate = s.currentTemplate;
      }
      ...
      updateLayers();
      this._flashSyncIndicator('synced');
      console.log('[LiveSync] State applied from peer, ts=' + data.ts);
    } catch(e) {
      console.error('[LiveSync] Failed to apply remote state:', e);
      this._flashSyncIndicator('error');
    } finally {
      // Always clear the guard, even if apply threw
      this._syncInProgress = false;
    }
  },
  ...
  _flashSyncIndicator(state) {
    ...
    if (state === 'syncing') {
      ...
      // Auto-recover error display after 4s
      this._indicatorTimer = setTimeout(() => {
        bar.className = 'collab-sync-bar';
        status.textContent = 'Synced';
      }, 4000);
    } else {
      // 'synced' — green, auto-fade after 2s
      bar.className = 'collab-sync-bar';
      status.textContent = 'Synced ✓';
      this._indicatorTimer = setTimeout(() => {
   ...
   * Coalesces rapid edits (e.g. typing) into a single push.
   */
  notifyChange() {
    if (!this.channel) return;
